...
<iframe id=foldFrame src="" style="display:none"></iframe>
...
<script language="javascript">
	...
	$(document).ready( function() {
		...
		//先将报表内容移动到隐藏的iframe中去，然后再调用easyui的parser来解析渲染页面，解析完后再将填报表内容移回来
		var div = document.getElementById( "report1_reportDiv" );
		$("<div id='div_report1'></div>").appendTo( $("#foldFrame").contents().find( "body" ) );
		$(div).children().appendTo( $("#foldFrame").contents().find( "#div_report1" ) );
		$.parser.parse();
		var tmpdiv = $("#foldFrame").contents().find( "#div_report1" );
