...
<%@ page import="java.io.*" import="java.text.*" pageEncoding="UTF-8"%>
<%@ page import="java.net.*" %>
<%
      String reportFileHome="/raqsoft/reportFiles";
 ...
 throws IOException
{
	...
	{
		spaceHtml.append("<img  src='"+contextPath+"/raqsoft/img/tree/space.gif'>");
	}
	...
	for(int i=0;i<fileList.length;i++)
	{
		...
		if(tmpFile.isDirectory()&&!tmpFile.isHidden())
		{
			...
			out.println("<img style='font-size:12px; font-family:宋体; color:black;cursor:hand' onclick=expandIt('"+menuID+"') id='"+menuID+"IMG' src='"+contextPath+"/raqsoft/img/tree/menu_closed.png' style='vertical-align:-3px;'>");
	...
	for(int i=0;i<fileList.length;i++)
	{
		...
		String menuID=parentMenuID+df.format(i);
		File tmpFile=fileList[i];
		 if(tmpFile.isFile()&&tmpFile.getName().endsWith(".rpx")&&!tmpFile.isHidden())
		{
			...
			out.println("<table border='0'  cellpadding='0' cellspacing='0'><tr height='10'><td height='10' nowrap align='left'>");
			out.println(spaceHtml.toString());
			out.println("<img id='"+menuID+"IMG' src='"+contextPath+"/raqsoft/img/tree/report.png' style='vertical-align:-3px;'>");
...
<%
			...
			out.println("<img style='font-size:12px; font-family:宋体; color:black;cursor:hand' onclick=expandIt('"+startMenuID+"') id='"+startMenuID+"IMG' src='"+request.getContextPath()+"/raqsoft/img/tree/menu_closed.png' style='vertical-align:-3px;'>");
...
function expandIt(blkObjName) {
	...
	if(selectDivObj.style.display=="none")
	{
	  for(n=blkObjName.length;n>4;n-=3)
	  {
	    ...
	    imgobj.src="<%=request.getContextPath()%>/raqsoft/img/tree/menu_open.png";
	  }
	}
	//否则隐藏之,显示上一级
	else
	{
	  for(n=blkObjName.length-3;n>4;n-=3)
	  {
	   ...
	   selectImgObj.src="<%=request.getContextPath()%>/raqsoft/img/tree/menu_closed.png";
	}
}
...
function showReport(menuID)
{
    ...
    var reportFile = selectRPTObj.getAttribute( "reportFile" );
    parent.contentArea.location.href="<%=request.getContextPath()%>/reportJsp/showReport.jsp?rpx="+reportFile;
}
