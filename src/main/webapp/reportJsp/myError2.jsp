...
$D();
sb.innerHTML='<%=ServerMsg.getMessage(request,"jsp.shrink")%>';
}else{
$D2();
sb.innerHTML='<%=ServerMsg.getMessage(request,"jsp.scanDetail")%>';
}
}
</script>
</head>
...
<%
	...
	out.println( "<h1>" + ServerMsg.getMessage(request,"jsp.errorInfo") + "：</h1><div style='color:red'>" + e.getMessage() + "</div>" );
	
%>
<span id="stateBut" onclick="$use()"><%=ServerMsg.getMessage(request,"jsp.scanDetail")%></span>
