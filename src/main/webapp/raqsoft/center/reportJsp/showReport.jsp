...
<%
	...
	for(int i = 0; i < jarr.length(); i++){
		...
		tempParam.append(name).append("=").append(valueObj).append(";");
	}
	...
	session.setAttribute("curr_context_param",ctx_param);
	...
	Context.getInitCtx().setMainDir( reportFileHome );//替换主路径！
	...
	if(paramNames!=null){
		...
		param.append(tempParam);
	}
	param.append(params);
	String resultPage = "queryReport.jsp?rpx=" + URLEncoder.encode( report, "UTF-8" ) + "&scroll=" + scroll+"&contextName=center_context";
...
<jsp:include page="toolbar.jsp?mobile=<%=mobile %>" flush="false" />
</div>
