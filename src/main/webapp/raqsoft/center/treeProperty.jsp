...
<TABLE style="margin:auto;width:500px" cellSpacing=0 cellPadding=0 border=0 class="layui-table">
 	...
 	<TR>
  	...
  	<TR>
  		<td>首页报表</td>
    	<TD>
     		<SELECT id=raqBox class="layui-input" style="WIDTH: 150px"> 
    		<OPTION value=""></OPTION>
    	<%
    ...
    <TR>
  	...
  	<TR>
	<TD>输入url</TD>
    	<TD><input type="textarea" class="layui-input" style="width:250px" onkeyup="urlReWrite();console.log(1);" id=urlBox value="<%=url%>"></TD>
    </TR>
  	<TR>
    	<TD colspan=2>(首页报表如果为空，则显示首页链接)</TD>
	</TR>
  	<TR>
...
<script language=javascript>
	...
	function submit() {
		...
		$.ajax({
			...
			success:function(strRet){
				...
				leftIFrameWin.location = "tree.jsp?status=" + s + "&position=" + leftIFrameWin.document.body.scrollTop + "&currId=0" ;
	...
	if( parent.parent.tree !=null ){
		var s = leftIFrameWin.tree_getNodeExpanded();
		leftIFrameWin.location.replace( "tree.jsp?status=" + s + "&position=" + leftIFrameWin.document.body.scrollTop + "&currId=0" );
	}
