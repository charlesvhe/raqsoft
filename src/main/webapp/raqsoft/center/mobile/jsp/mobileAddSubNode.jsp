...
com.raqsoft.center.entity.User[] user = cfg.getUsers();
...
<script language="javascript">	
	...
	var inputHome = '<%=inputHome%>';
	 ...
	 function lookoverdir(appendToItem){
		 $.ajax({
				type:"post",
				...
				success:function(strRet){
					...
					$(appendToItem).append("<option value='' style='color:blue'>填报根路径</option>");
					 $.each(jsonArr, function(o1, o2) {
						 $(appendToItem).append("<OPTION value=\""+o2+"\">"+o2+"</OPTION>");
					});
	...
	function showSpan( span ) {
	 ...
	 function urlReWrite(){
		...
		$(function(){layuiParse();parseRadio();})
...
<div class="layui-row layui-col-space30">
    <div class="layui-col-xs5">
		      <div class="grid">
		      <TABLE lay-skin="nob" style="border: none;width:340px" class="layui-table layui-bg-gray" cellSpacing=0 cellPadding=0 align=center>
			 	<colgroup>
			 		<col width="150px"/>
			 		<col/>
			 	</colgroup>
			 ...
			 <TR>	
		    	<TD>节点名称</TD>
		    	<TD><INPUT id=labelBox class="layui-input" style="VERTICAL-ALIGN: middle; WIDTH: 145px;"></TD>
			</TR>
			<tr><td colspan="2" >
		  	...
		  	<TR class="reportSpan businessInput">
		    	...
		    	<TD><SELECT id=raqBox onchange="rpgHide(this);shtHide(this);" class=layui-input style="WIDTH: 145px;"> 
		    	<%
		    	...
		    	<td><input style="margin-left:-10px" type="button" class="layui-btn layui-bg-black layui-btn-sm" onclick="addReport('report')" value="上传"/>
		    	</td>
		    	</TD></TR>
		    ...
		    <TR class="querySpan">
		    	...
		    	<TD colspan=3>
			    	<form name="radioForm">
				    	<table style="width=100%">
					    	...
					    	<tr><td></td><td>
					    		<input type="radio" class="analyseType" onclick="hideSpan( 'analysisSpan0' );showSpan( 'analysisSpan2' );anaBox.value='';" name="analyseType" value="ql"/>数据源+</br>查询语句
					    	</td><td>
		    	...
		    	<td>编写dfx脚本</td>
		    	<td colspan=2>
		    	<textarea class="layui-textarea" id="dfxScriptBox"></textarea>
		    	</td>
		    	</tr>
...
<div class="layui-container layui-bg-gray" style="top:60px;height:100%"> 
      <div class="layui-row layui-col-space30" style="margin-top:20px">
		    <div class="layui-col-xs6" style="font-weight: bold;">
		    浏览权限    ——————————————————————————
		    </div>
    </div>
    <div class="layui-row layui-col-space30">
    ...
    <input type="button" value="保存" onclick="submitNode();"id="loginbtn" style="margin-top:0px;" class="layui-btn layui-btn-sm"/>
  	</div> -->
...
$(function(){
	...
	if(isModify){
		...
		typeChanged();
		<%
		if(anaFile != null && !"dfxScript".equals(analyseType) && anaFile.toLowerCase().startsWith("web-inf")){
			if(anaFile.indexOf('/') >= 0){
				byte rec = 0;
				while(rec < 2){
					anaFile = anaFile.substring(anaFile.indexOf('/')+1);
					rec++;
				}
			}
		}
