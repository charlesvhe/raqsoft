...
function queryData(reQuery,dataId,type,dataSource,ql,dfxFile,dfxScript,dfxParams,inputFiles,callback,filter,currTable) {
	$.ajax({
		...
		data: {action:2,oper:'query',reQuery:reQuery,dataId:dataId,type:type,dataSource:dataSource,ql:ql,dfxFile:dfxFile,dfxScript:dfxScript,dfxParams:dfxParams,inputFiles:inputFiles,cursorSize:guideConf.cursorSize,filter:filter,currTable:currTable,dataFileType:guideConf.dataFileType},
...
function doCalcDfxData(conf) {
	...
	var success = function(data){
		...
		$('iframe[confName="'+conf.name+'"]').attr('src', contextPath+guideConf.reportPage+"?reportId="+conf.reportId+"&guideDir="+guideConf.guideDir+"&isOlap=no"+"&confName="+encodeURIComponent(conf.name)+"&finish="+finish);
		return;
		//
		var form = $('#frameForm');
		if (form.length == 0) {
...
function reportCellDrill(reportConfName, topValues, leftValues, fieldValues, detail) {
	if (guideConf.view == 'analyse')
	{
		...
		{
			var whereConf = [{"level":1,"fieldInfo":transWhereInfo(conf2.tops[i].src,null,false),"oper":"等于","values":topValues[i],"disp":""}];
		...
		{
			var whereConf = [{"level":1,"fieldInfo":transWhereInfo(conf2.lefts[i].src,null,false),"oper":"等于","values":leftValues[i],"disp":""}]
