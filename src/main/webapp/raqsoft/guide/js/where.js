...
var whereUtils = {
	...
	refresh : function(fields, initField, wheres,h) {
		...
		for (var i=0; i<wheres.length; i++) {
			whereUtils.addRow(cols,wheres[i].level,i,false,h);
			if (wheres[i].join) continue;
			if (!cache.where.less) {
				var toLevel = 0;
				...
				for (var j=wheres[i].level; j>toLevel; j--) {
					whereUtils.addRow(cols,j,i,true,h);
				}
			}
		}
	...
	//
	,addRow : function(cols,level,index,andOr,h) {//,join,fieldInfo,oper,values
		//if (index>0)
		...
		if (wheres[index].join) {
			...
			var fieldChange = function(thisInfos){
				...
				//var edit = having?"":thisInfos.edit;
	...
	,getExp : function(conf, add, type, expType,field) {//conf=fi.where.conf
		...
		for (var i=0; i<conf.length; i++) {
			...
			if(field && field.calc != null) e = cif.exp;//"("+field.calc+")";
		...
		{type:2,options:[
			{name:resources.guide.js25,html:function(){
					...
					if (r.indexOf(',') > 0){
						...
						var isnan = isNaN(r);
						if(isnan || typeof r == "string" || typeof r == "string") return "_x_='" + r+"'";
						else return '_x_=' + r;
					}
				},dfxExp:function(values){
					var vs = values.split(',');
			...
			{name:resources.guide.js35,noDim:1,html:function(){
					...
					for (i=0; i<vs.length; i++) {
						if (i>0) r += ' OR ';
						r += props._like_prefix + vs[i] + props._like_suffix;
						//alert(r);
					}
					return r;
				},dfxExp:function(values){
			...
			{name:resources.guide.js36,noDim:1,html:function(){
					...
					for (i=0; i<vs.length; i++) {
						if (i>0) r += ' AND ';
						r += props._notlike_prefix + vs[i] + props._notlike_suffix;
					}
					return r;
				},dfxExp:function(values){//不包含,位置为空
					var vs = values.split(',');
					var r = '';
					for (i=0; i<vs.length; i++) {
						if (i>0) r += ' && ';
//						r += '!like(_x_,"*'+vs[i]+'*")'
			...
			{name:resources.guide.js37,noDim:1,html:function(){
					...
					for (i=0; i<vs.length; i++) {
						if (i>0) r += ' OR ';
						//r += "_x_ like '" + vs[i] + "%'";
						r += props._startfrom_prefix + vs[i] + props._startfrom_suffix;
					}
					return r;
				},dfxExp:function(values){
					var vs = values.split(',');
			...
			{name:resources.guide.js38,noDim:1,html:function(){
					...
					for (i=0; i<vs.length; i++) {
						if (i>0) r += ' OR ';
						//r += "_x_ like '%" + vs[i] + "'";
						r += props._endat_prefix + vs[i] + props._endat_suffix;
					}
					return r;
				},dfxExp:function(values){
					var vs = values.split(',');
		...
		{type:3,options:[
			{name:resources.guide.js25,html:function(){
					...
					return "_x_="+props._date_prefix + values[0] + props._date_suffix;
				},dfxExp:function(values){
			...
			{name:resources.guide.js43,html:function(){
					...
					return "_x_="+props._date_prefix + dateUtils.getToday() + props._date_suffix;
				},dfxExp:function(values){
			...
			{name:resources.guide.js44,html:function(){
					...
					return '_x_>='+ props._date_prefix + dateUtils.getWeekBegin() + props._date_suffix 
					+' AND _x_<='+props._date_prefix + dateUtils.getWeekEnd() + props._date_suffix ;
				},dfxExp:function(values){
			...
			{name:resources.guide.js45,html:function(){
					...
					//props._date_prefix + dateUtils.getMonthEnd() + props._date_suffix
					return "_x_>="+props._date_prefix + dateUtils.getMonthBegin() + props._date_suffix
					+" AND _x_<="+props._date_prefix + dateUtils.getMonthEnd() + props._date_suffix;
				},dfxExp:function(values){
			...
			{name:resources.guide.js46,html:function(){
					...
					props._date_prefix + dateUtils.getYearEnd() + props._date_suffix
					return "_x_>="+props._date_prefix + dateUtils.getYearBegin() + props._date_suffix
					+" AND _x_<="+props._date_prefix + dateUtils.getYearEnd() + props._date_suffix;
				},dfxExp:function(values){
		...
		{type:4,options:[
			...
			{name:resources.guide.js26,html:function(){
					...
					return "_x_<>"+ props._timestamp_prefix + values[0] +  props._timestamp_suffix;
				},dfxExp:function(values){
					return '_x_!=time("' + values + '")';
				}
			},
		...
		{type:5,options:[
			...
			{name:resources.guide.js43,html:function(){
					...
					return "_x_="+ props._timestamp_prefix + dateUtils.getToday() +  props._timestamp_suffix;
				},dfxExp:function(values){
					return '_x_==date("' + dateUtils.getToday() + '")';
				}
			},
			{name:resources.guide.js44,html:function(){
					...
					return "_x_>="+ props._timestamp_prefix + dateUtils.getWeekBegin() + props._timestamp_suffix
					+" AND _x_<="+props._timestamp_prefix + dateUtils.getWeekEnd() + props._timestamp_suffix;
				},dfxExp:function(values){
					return '_x_>=date("' + dateUtils.getWeekBegin() + '") && _x_<=date("' + dateUtils.getWeekEnd() + '")';
				}
			},
			{name:resources.guide.js45,html:function(){
					...
					return "_x_>="+ props._timestamp_prefix + dateUtils.getMonthBegin() + props._timestamp_suffix
					+" AND _x_<="+props._timestamp_prefix + dateUtils.getMonthEnd() + props._timestamp_suffix;
				},dfxExp:function(values){
					return '_x_>=date("' + dateUtils.getMonthBegin() + '") && _x_<=date("' + dateUtils.getMonthEnd() + '")';
				}
			},
			{name:resources.guide.js46,html:function(){
					...
					return "_x_>="+props._timestamp_prefix + dateUtils.getYearBegin() + props._timestamp_suffix 
					+" AND _x_<="+props._timestamp_prefix + dateUtils.getYearEnd() + props._timestamp_suffix;
				},dfxExp:function(values){
					return '_x_>=date("' + dateUtils.getYearBegin() + '") && _x_<=date("' + dateUtils.getYearEnd() + '")';
				}
			},
