...
artDialog.fn = artDialog.prototype = {
	...
	focus: function () {
		...
		try {
			...
			if (initRpxsOver && initDialogOver) {
				aly.rpxFocusChanged();
			}
		} catch (e) {};
...
_use = function (event) {
	...
	_dragEvent.onmove = function (x, y) {
		if (isResize) {
			...
			//xingjl 2016/08/19
			try {
				if (artDialogMaxTop !== undefined) {
					if (top<artDialogMaxTop) top = artDialogMaxTop; 
				}
			} catch (e) {}
	...
	_dragEvent.onend = function (x, y) {
		...
		try {			
			...
			for (var i=0; i<reports.length; i++) {
	        	if (reports[i].dlg.DOM.wrap == wrap) {
