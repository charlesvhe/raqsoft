...
//		if (areas == "" || !areas) areas = "items;reports;reportConf";
		...
		addReport.click(function(){
			...
			selDom1.attr('disabled',true).css({'background-color':'#41455A','border':'0','color':'#bbb','padding':'4px','margin-top':'4px','width':'260px','height':'28px'}).attr('title','').change(function(){
			});
...
function doCalcDfxData(conf) {
	...
	var success = function(data){
		...
		var url = contextPath+guideConf.reportPage+"?reportId="+conf.reportId+"&guideDir="+guideConf.guideDir+"&isOlap=no";
		...
		if (conf.tops.length==0&&conf.lefts.length==0) {
			...
			if (conf.isRowData) {
				...
				for (var i=0; i<fields.length; i++) {
					rpt.cells.push({row:1,col:i+1,row2:1,col2:i+1,format:'',value:fields[i].name,valueExp:'',extend:16,leftMain:'',topMain:'',tip:5,adjustSizeMode:48,textWrap:1,hAlign:209,color:'',backColor:'230,244,255'});
					rpt.cells.push({row:2,col:i+1,row2:2,col2:i+1,format:fields[i].format,value:'',valueExp:rpx.exps(conf,fields[i],i==0?3:5,i==0?orders:''),extend:16,leftMain:'',topMain:'',tip:4,adjustSizeMode:48,textWrap:1,hAlign:208,color:'',backColor:''});
				}
				rpt.rowCount = 2;
				...
				for (var i=0; i<fields.length; i++) {
					rpt.cells.push({row:i+1,col:1,row2:i+1,col2:1,format:'',value:fields[i].name,valueExp:'',extend:16,leftMain:'',topMain:'',tip:5,adjustSizeMode:48,textWrap:1,hAlign:209,color:'',backColor:'230,244,255'});
					rpt.cells.push({row:i+1,col:2,row2:i+1,col2:2,format:fields[i].format,value:'',valueExp:rpx.exps(conf,fields[i],i==0?3:5,i==0?orders:''),extend:19,leftMain:'',topMain:'',tip:4,adjustSizeMode:48,textWrap:1,hAlign:208,color:'',backColor:''});
				}
			...
			if (conf.isRowData) {
				...
				if (rpt.rowCount>1 && conf.lefts.length>0) {
					...
					rpt.cells.push({row:1,col:1,row2:rpt.rowCount-1,col2:conf.lefts.length,format:'',value:value11,diagonal:36,valueExp:'',extend:16,leftMain:'',topMain:'',tip:6,adjustSizeMode:48,textWrap:1,hAlign:209,color:'',backColor:'230,244,255'});
				}
				for (var i=0; i<conf.lefts.length; i++) {
					rpt.cells.push({row:rpt.rowCount,col:i+1,row2:rpt.rowCount,col2:i+1,format:conf.lefts[i].format,value:'',valueExp:rpx.exps(conf,conf.lefts[i],2),extend:18,leftMain:'',topMain:'',tip:1,adjustSizeMode:48,textWrap:1,hAlign:208,color:'',backColor:'230,244,255'});
				}
				...
				for (var i=maxTop; i>=0; i--) {
					...
					for (var j=0; j<fields.length; j++) {
						...
						counti++;
						if (showAggrTitle) {
							rpt.cells.push({row:i+1,col:currCol,row2:rpt.rowCount-1,col2:currCol,format:'',value:fields[j].name,valueExp:'',extend:16,leftMain:'',topMain:'',tip:7,adjustSizeMode:48,textWrap:1,hAlign:209,color:'',backColor:'230,244,255'});
						}
						...
						var cell = {row:rpt.rowCount,col:currCol,row2:rpt.rowCount,col2:currCol,format:fields[j].format,value:'',valueExp:exp,extend:16,leftMain:leftMain,topMain:topMain,tip:3,adjustSizeMode:48,textWrap:1,hAlign:208,color:'',backColor:backColors[j]};
						finalCells.push(cell);
						rpt.cells.push(cell);
					}
					if (i>0) rpt.cells.push({row:i,col:conf.lefts.length+1,row2:i,col2:fields.length==0?currCol+1:currCol,format:conf.tops[i-1].format,value:'',valueExp:rpx.exps(conf,conf.tops[i-1],2),extend:19,leftMain:'',topMain:'',tip:2,adjustSizeMode:48,textWrap:1,hAlign:209,color:'',backColor:'230,244,255'});
				}
			...
			//纵向显示数据
				...
				if (rpt.colCount>1 && conf.tops.length>0) {
					...
					if (fields.length == 1) aggrNames = ","+fields[0].name;
					var value11 = leftNames + "" + topNames + "" + aggrNames;
					rpt.cells.push({row:1,col:1,row2:conf.tops.length,col2:rpt.colCount-1,format:'',value:value11,diagonal:36,valueExp:'',extend:16,leftMain:'',topMain:'',tip:6,adjustSizeMode:48,textWrap:1,hAlign:209,color:'',backColor:'230,244,255'});
				}
				for (var i=0; i<conf.tops.length; i++) {
					rpt.cells.push({row:i+1,col:rpt.colCount,row2:i+1,col2:rpt.colCount,format:conf.tops[i].format,value:'',valueExp:rpx.exps(conf,conf.tops[i],2),extend:19,leftMain:'',topMain:'',tip:2,adjustSizeMode:48,textWrap:1,hAlign:209,color:'',backColor:'230,244,255'});
				}
				...
				for (var i=maxLeft; i>=0; i--) {
					...
					for (var j=0; j<fields.length; j++) {
						...
						counti++;
						if (showAggrTitle) {
							rpt.cells.push({row:currRow,col:i+1,row2:currRow,col2:rpt.colCount-1,format:'',value:fields[j].name,valueExp:'',extend:16,leftMain:'',topMain:'',tip:7,adjustSizeMode:48,textWrap:1,hAlign:209,color:'',backColor:'230,244,255'});
						}
						finalFields.push(fields[j]);
						var exp = "";
						...
						var cell = {row:currRow,col:rpt.colCount,row2:currRow,col2:rpt.colCount,format:fields[j].format,value:'',valueExp:exp,extend:16,leftMain:leftMain,topMain:topMain,tip:3,adjustSizeMode:48,textWrap:1,hAlign:208,color:'',backColor:backColors[j]};
						finalCells.push(cell);
						rpt.cells.push(cell);
					}
					if (i>0) rpt.cells.push({row:conf.tops.length+1,col:i,row2:fields.length==0?currRow+1:currRow,col2:i,format:conf.lefts[i-1].format,value:'',valueExp:rpx.exps(conf,conf.lefts[i-1],2),extend:18,leftMain:'',topMain:'',tip:1,adjustSizeMode:48,textWrap:1,hAlign:209,color:'',backColor:'230,244,255'});
				}
				for (var i=0; i<finalFields.length; i++) {
					if (finalFields[i]._fieldType != 'analyse') continue;
					var aggrCell = "";
...
function reportCellDrill(reportConfName, topValues, leftValues, fieldValues, detail) {
	...
	if (detail) {
		for (var i=0; i<rpxData.items.length; i++) {
			...
			var nti = {name:ii.name,srcItems:[ii.id],exp:"itemId0",aggrExp:"",use:1,order:0,gourps:[],analyse:null,format:'',dataType:''};
			//lefts:[{name:'',macroName:'',srcItems:[itemId0,itemId1],exp:'itemId0/itemId1',aggrExp:'sum(itemId0)/sum(itemId1)',use:1,order:0无序/1升序/2降序,groups:[lefts,tops里的分组，空分组表示整体聚合]/null表示随分组自动聚合,analyse:{analyseName:'占比/排名/比上期/比同期',field:'被分析的测度字段',scopeGroups:[空则表示针对全部]},format:'',dataType:'',_parentType:'top/left/field',_fieldType:'group/detail/aggr/analyse',_status:'为空表示正确，不为空是失效的具体信息'}]
		...
		for (var i=topValues.length; i<conf.tops.length; i++) {
			var ti = conf.tops[i];
			var nti = {name:ti.name,srcItems:[],exp:ti.exp,aggrExp:ti.aggrExp,use:ti.use,order:ti.order,gourps:[],analyse:null,format:ti.format,dataType:ti.dataType};
