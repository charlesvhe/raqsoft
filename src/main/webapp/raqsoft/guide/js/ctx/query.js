		...
		$.fn.zTree.init(treeDiv, {
			...
			callback: {
				...
				,onDrop: function(e, treeId, treeNodes, targetNode, moveType) {
					...
					if (aly.cache.doms.div4){
						aly.cache.doms.div4.unbind('mouseover').unbind('mouseout');
						//alert(aly.cache.doms.div2.find('#placeHolders').css('display'));
						if (aly.cache.doms.div4.find('#placeHolders').css('display') == 'block'){
							pos = 'params';
						}
						aly.cache.doms.div4.find('#placeHolders').css('display','none');
					}
...
//							else aggr = 'count';
							if (get5TypesByDTType(iiObj.finalType) == 1) aggr = 'sum';
							else aggr = 'count';
						}
						if(pos == 'params') aggr='';
						//alert(aggr);
...
function submitQuery() {
	...
	if(guideConf.dashboardCtx) rqAnalyse.rpxs = [];
	var form = $('<form method="post" accept-charset="UTF-8"></form>');
...
function submitAnalyse() {
	...
	form.attr('target', '_blank');
	form.append('<input type="hidden" name="olap" value="'+aly.toString(rqAnalyse)+'">');
	form.append('<input type="hidden" name="resetLmd" value="no">');
	$('body').append(form);
	form[0].submit();
}
...
$(document).ready(function(){
	...
	if ("olap" != guideConf.view) {
		...
		if (dqlQuery.lmd == null) {
			if(noGlmdFile) {
				alert("No glmd file opened");
			}else alert(resources.guide.js79.replaceAll('{0}',guideConf.dataSource));
			return;
		}
		...
		var l_d = 8;
		if(guideConf.dashboardCtx) l_d = 14;
		for (var i=1; i<=8; i++) {
...
//		});
		...
		if (olapStr != '') {
			...
			var type = 0;
			if(guideConf.dashboardCtx) type = 6;
					...
					,dataSets:[
						{
							...
							,dqlConf:{}
							,dataId:guideConf.dataFolderOnServer+'q'+new Date().getTime()+"."+(guideConf.dataFileType=='text'?'txt':'bin')//缓存的数据ID//缓存的数据ID
		...
		for (var i=0; i<rqAnalyse.dataSets.length; i++){
			...
			if (dsi.type == 6) {
				...
				if (dsi.fields == null) {
					...
					olapObj.server({action:2,oper:'getTableInfo',isGlmd:"true",sqlId:dsi.sqlId,sql:currsd.sql,dataSource:currsd.dataSource,scanRow:guideConf.scanRow,isCursor:guideConf.dataFileType=='text'?2:1},function(data){
							if (data.indexOf('error:')==0) {
...
function manageDataSet(){
	...
	if(guideConf.dashboardCtx)  $('#dataSetsDiv').find('img').hide();
	...
	if(!guideConf.dashboardCtx) $('#dataSetsDiv').append(addBut);
	addBut.click(function(){
		...
		if(!guideConf.dashboardCtx) $(this).before(nd);
