...
var whereUtils = {
	...
	,addRow : function(cols,level,index,andOr) {//,join,fieldInfo,oper,values
		...
		if (wheres[index].join) {
			...
			var fieldChange = function(thisInfos){
				...
				//组表dql类型转换
				var dtype = thisInfos.dataType;
				dtype = get5TypesByDTType(dtype);
				var typeObj = whereUtils.getType(dtype);//(iObj.finalType);
				//alert(currWhereField4Having + "--" + currWhereField._type);
				if (typeObj == null) {
					alert('unavailable type[' + dtype + ']!');
					return;
				}
	...
	,getDisp : function(conf) {
		...
		for (var i=0; i<conf.length; i++) {
			...
			var opt = whereUtils.getOption(get5TypesByDTType(cif.dataType),ci.oper);
	...
	,getExp : function(conf, add, type, expType, field, fieldType) {//conf=fi.where.conf
		...
		for (var i=0; i<conf.length; i++) {
			...
			var exp = "";
			//2019.5.13判断类型，组表dql区分处理;2019.5.22类型转换了，不需要在判断
//			if(fieldType && (ci.oper == resources.guide.js25 || ci.oper == resources.guide.js26)){
//				var cve = getTypedExp(ci.values,fieldType);
//			}
			var cve;
			if(!cve) cve = ci.values;
			if (expType == 1) exp = opt.exp(null,[cve]);
			else exp = opt.dfxExp(cve);
			var e = cif.exp;
			if(field && field.aggr != "") e = cif.disp;
	    ...
	    {type:1,options:[
			{name:resources.guide.js25,html:function(){
					...
					if (values[0].indexOf(',') > 0) {
						...
						var vstr = "";
						for(var vn = 0; vn < vs.length; vn++){
							if(vn > 0) vstr += ",";
							var vsn = vs[vn].replaceAll("'","");
							try{
								vstr += parseInt(vsn);
							}catch(e){
								vstr += '"'+vsn+'"';
...
//						r += '!like(_x_,"*'+vs[i]+'*")'
		...
		{type:3,options:[
			{name:resources.guide.js25,html:function(){
					...
					if (dimType>0) return '_x_=' + translateDimExp(dimType, dimExp, values[0]);
					return "_x_=DATE(" + dateValueToLong(values[0]) + ")";
				},dfxExp:function(values){
			...
			{name:resources.guide.js43,html:function(){
					...
					return "_x_=date(" + dateValueToLong(dateUtils.getToday()) + ")";
				},dfxExp:function(values){
			...
			{name:resources.guide.js44,html:function(){
					...
					return '_x_>=date(' + dateValueToLong(dateUtils.getWeekBegin()) + ') AND _x_<=date(' + dateValueToLong(dateUtils.getWeekEnd()) + ')';
				},dfxExp:function(values){
			...
			{name:resources.guide.js45,html:function(){
					...
					return "_x_>=date(" + dateValueToLong(dateUtils.getMonthBegin()) + ") AND _x_<=date(" + dateValueToLong(dateUtils.getMonthEnd()) + ")";
				},dfxExp:function(values){
			...
			{name:resources.guide.js46,html:function(){
					...
					return "_x_>=date(" + dateValueToLong(dateUtils.getYearBegin()) + ") AND _x_<=date(" + dateValueToLong(dateUtils.getYearEnd()) + ")";
				},dfxExp:function(values){
		...
		{type:4,options:[
			{name:resources.guide.js25,html:function(){
					...
					return "_x_="+valueToTranslateFunc(values[0]);
				},dfxExp:function(values){
		...
		{type:5,options:[
			...
			{name:resources.guide.js43,html:function(){
					...
					return "_x_="+valueToTranslateFunc(dateUtils.getToday());
				},dfxExp:function(values){
					return '_x_==date("' + dateUtils.getToday() + '")';
				}
			},
			{name:resources.guide.js44,html:function(){
					...
					return "_x_>="+valueToTranslateFunc(dateUtils.getWeekBegin()) + " AND _x_<="+valueToTranslateFunc(dateUtils.getWeekEnd());
				},dfxExp:function(values){
					return '_x_>=date("' + dateUtils.getWeekBegin() + '") && _x_<=date("' + dateUtils.getWeekEnd() + '")';
				}
			},
			{name:resources.guide.js45,html:function(){
					...
					return "_x_>="+valueToTranslateFunc(dateUtils.getMonthBegin()) + " AND _x_<="+valueToTranslateFunc(dateUtils.getMonthEnd());
				},dfxExp:function(values){
					return '_x_>=date("' + dateUtils.getMonthBegin() + '") && _x_<=date("' + dateUtils.getMonthEnd() + '")';
				}
			},
			{name:resources.guide.js46,html:function(){
					...
					return "_x_>="+valueToTranslateFunc(dateUtils.getYearBegin()) + " AND _x_<="+valueToTranslateFunc(dateUtils.getYearEnd());
				},dfxExp:function(values){
					return '_x_>=date("' + dateUtils.getYearBegin() + '") && _x_<=date("' + dateUtils.getYearEnd() + '")';
				}
			},
...
function getTypedExp(v,type){
	if(type == '10'){
		try{
			var d = new Date(v);
			if(d && d instanceof Date) return 'date(\''+v+'\')';
		}catch(e){
			alert("数据类型不匹配");
		}
	}else if(type == '11'){
...
const java_sql_Types_INTEGER = 4;
const java_sql_Types_SMALLINT = 5;
const java_sql_Types_BIGINT = -5;
const java_sql_Types_TINYINT = -6;
const java_sql_Types_FLOAT = 6;
const java_sql_Types_REAL = 7;
const java_sql_Types_DOUBLE = 8;
const java_sql_Types_DECIMAL = 3;
const java_sql_Types_NUMERIC = 2;
const java_sql_Types_CHAR = 1;
const java_sql_Types_VARCHAR = 12;
const java_sql_Types_LONGVARCHAR = -1;
const java_sql_Types_DATE = 91;
const java_sql_Types_TIME = 92;
const java_sql_Types_TIMESTAMP = 93;
const java_sql_Types_BOOLEAN = 16;
const java_sql_Types_BINARY = -2;
const java_sql_Types_VARBINARY = -3
const java_sql_Types_LONGVARBINARY = -4;
const java_sql_Types_BLOB = 2004;
...
const DT_DATE =  8;
const DT_TIME =  9;
const DT_DATETIME =  10;
const DT_STRING =  11;
const DT_BOOLEAN =  12;
const DT_INT_SERIES =  51;
const DT_LONG_SERIES =  52;
const DT_SHORT_SERIES =  53;
const DT_BIGINT_SERIES =  54;
const DT_FLOAT_SERIES =  55;
const DT_DOUBLE_SERIES =  56;
const DT_DECIMAL_SERIES =  57;
const DT_DATE_SERIES =  58;
const DT_TIME_SERIES =  59;
const DT_DATETIME_SERIES =  60;
const DT_STRING_SERIES =  61;
const DT_BYTE_SERIES =  62;

function get5TypesByDTType(type){
...
function valueToTranslateFunc(v){
	let trans = null;
	v = v.replace(/-/g, '/');
	try{
		let date = new Date(v);
		trans = "DATETIME("+date.getTime()+")"
	}catch(e){
		console.warn("不是日期时间格式");
	}
