...
function DqlQuery() {
	...
	var mdUtils = {
		...
		getFKByLastField : function(t, fkf) {
			var t = mdUtils.getTable(t);
			if (t.fks) {
				for (var i=0; i<t.fks.length; i++) {
					if (t.fks[i].fields[0] == fkf) return t.fks[i];
				}
			}
		},
		//获得表的主键列表
		...
		*/
		...
		getFieldInfos : function(parent, fields, levelCount, table, field, containPk) {
			...
			if (levelCount > 1) {
				...
				var fkObj = mdUtils.getFKByLastField(p.lastTable, p.lastField);
				if (fkObj != null) {
					//if (fkObj.fields.length == 1) return;2019.4.8查询页面拖拽外键字段，找不到广义字段，不显示
		...
		generateDql : function(currDomInfos, silence, top) {
			...
			*/
			...
			for (var i=0; i<fields.length; i++) {
				...
				dqlFields.push({name:fDom.name,dataType:iObj.finalType,edit:defaultEdit.autoEditStyle(get5TypesByDTType(iObj.finalType),'',iObj.dim)});
				...
				if (wDom && (wDom.conf.length>0 || wDom.where != '')) {
					var where = whereUtils.getExp(wDom.conf, fDom.tAlias+".", 1, null,null,iObj.finalType);
					if (where == '') {
			...
			$.ajax({
				...
				data: {action:2,isGlmd:isGlmd,oper:'getDql',dataSource:guideConf.dataSource,dqlSegments:JSON.stringify(sgts).replaceAll('"',"<d_q>"),filter:guideConf.filter},
...
function Olap(){
	...
	obj.getConfStr = function(layout) {
	...
	obj.rpxUtils = {
		...
		,getDqlSegments4dbd : function(conf,params) {
			var rd = rqAnalyse;
			...
			for (var i=0; i<fields.length; i++)
			{
				...
				var iObj = dqlQuery.confUtils.getInfosObj(fieldi.autoSrc == null || fieldi.src != fieldi.autoSrc ? fieldi.src : fieldi.autoSrc);
...
//						}
				...
				if(modelHasAggrField && modelRpxAggrDim == fieldi.dim){
					tj.bys.push(iObj.expNoTableNoAggr);
				}
				/*
				var subTable = iObj.subTable;
				if (subTable != '') subTable = "@"+subTable;
				...
				if (conf.type == 1 || conf.type == 2) {
					...
					for (var j=0; j<fs.length; j++) {
						...
						if (hasGroup ){
							...
							aggrs += aggr + "(" + e + "):" + n;
							if (fj.exp == '' || !fj.exp) srcTypes += "," + srcn + ":" + fs[j].dataType;
		...
		if (conf.tops.length==0&&conf.lefts.length==0) {
			...
			if (conf.isRowData) {
				...
				for (var i=0; i<fields.length; i++) {
					...
					rpt.cells.push({row:2,col:i+1,row2:2,col2:i+1,format:fields[i].format,value:'',valueExp:getExps(conf,fields[i],i==0?3:5,i==0?orders:''),extend:16,leftMain:'',topMain:'',tip:4,adjustSizeMode:48,textWrap:1,hAlign:zhibiao[0].hAlign,color:zhibiao[0].color,backColor:zhibiao[0].backColor,dispExp:defaultEdit.getEditExp4Rpx(fields[i].srcEdit,get5TypesByDTType(fields[i].dataType)==1),formatExp:formatExp});
