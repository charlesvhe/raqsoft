...
//	}
	...
	,set : function(str) {
		...
		if(guideConf.dashboardCtx){
			if(guideConf.openDOlap){
				dbdInit(olapObj.layout);
			}
		}
	}
	,toString : function(rqa) {
		if (rqa) {
		...
		} else if(guideConf.dashboardCtx) {
			var layout = {
				params : aly.params,
			...
			return olapObj.getConfStr(layout);
		}else return olapObj.getConfStr(); //oldctx
	}
	...
	,showDataSetRpxs : function(dataSet,lastRpxFunc) {
		var ds = aly.getDataSet(dataSet);
		//console.log(guideConf.showHistoryRpx);
		...
		if (guideConf.showHistoryRpx != 'yes'){
			...
			if (ds.type!=6 && ds.fields == null) {
				olapObj.server({action:2,oper:'getTableInfo',isGlmd:"true",dataId:ds.dataId,dataFileType:guideConf.dataFileType,scanRow:guideConf.scanRow,isCursor:guideConf.dataFileType=='text'?2:1},function(data){
						if (data.indexOf('error:')==0) {
							ds._status = data.substring(6);
							return;
						}
	...
	,refreshAll : function(){
		...
		rqAnalyse.currRpx = cr;
	}
	,refresh : function(noCalc, noRefreshDialog, drillRpx, noAggr, cb) {
		...
		for (var i=rqAnalyse.rpxs.length-1; i>=0; i--) {
			...
			if(guideConf.dashboardCtx){
				...
				confsDiv.append("<div style='padding:5px;cursor:pointer;"+sty+"' confName='"+confi.name+"' title='"+rowNum+"_"+colNum+"'><img style='vertical-align:-2px;cursor: pointer;' lock=1 src='"+contextPath+(guideConf.guideDir+(confi.autoCalc==1?"/img/guide/17.png":"/img/guide/18.png"))+"'><span style='padding:0 20px 0 5px;vertical-align:1px;'>"+confi.name+"</span><img style='float:right;vertical-align:-2px;cursor:pointer;margin:0 5px;' del=1 src='"+contextPath+guideConf.guideDir+"/img/guide/13.png'><img modify=1 style='vertical-align:-2px;cursor:pointer;margin:0 5px;float:right;' src='"+contextPath+guideConf.guideDir+"/img/guide/31.png'></div>")
			}
			else confsDiv.append("<div style='padding:5px;cursor:pointer;"+sty+"' confName='"+confi.name+"'><img style='vertical-align:-2px;cursor: pointer;' lock=1 src='"+contextPath+(guideConf.guideDir+(confi.autoCalc==1?"/img/guide/17.png":"/img/guide/18.png"))+"'><span style='padding:0 20px 0 5px;vertical-align:1px;'>"+confi.name+"</span><img style='float:right;vertical-align:-2px;cursor:pointer;margin:0 5px;' del=1 src='"+contextPath+guideConf.guideDir+"/img/guide/13.png'><img modify=1 style='vertical-align:-2px;cursor:pointer;margin:0 5px;float:right;' src='"+contextPath+guideConf.guideDir+"/img/guide/31.png'></div>")
		}
		...
		if(guideConf.dashboardCtx) confsDiv.find('img[del=1]').hide();//隐藏删除数据集按钮
		...
		addReport.click(function(){
			...
			var selDom1 = getSelectDom(existRpx.length==0?[""]:existRpx, existRpx.length==0?[resources.guide.js120]:existRpx,"" );
		...
		if (conf.type == 1) { //自定义报表
			...
			if(guideConf.dashboardCtx) 
				tb += +'</tr>'
				+'<tr>'
				+'<td colspan="2" style="border:1px solid #E4E4E4;"><div cType=5></div></td>'
				+'<td colspan="1" style="border:1px solid #E4E4E4;"><div cType=5></div></td>';
			tb += '</tr>'+'</tbody>';
			var tbody = $(tb);
			table.append(tbody);
			...
			if(guideConf.dashboardCtx) div4 = $(tds[7]).find('div[cType]');
...
//			}
			...
			var styles = getSelectDom(existRpx.length==0?[""]:existRpx, existRpx.length==0?[resources.guide.js129]:existRpxNames,"" );
			...
			if(!guideConf.dashboardCtx) styles.val(conf.template);
		...
		var changeFunc = function( event, ui ) {
			...
			if(guideConf.dashboardCtx) div4.find('#confHints').css("display",div4.find("div[iType=7]").length-div4.find('.ui-sortable-helper').length>0?"none":"block");
			...
			if (conf.type == 1) {
				...
				if(guideConf.dashboardCtx) div4.parent().attr('sel',0).css("background-color","");
				//console.log("----1");
		...
		try {
		var t = 3;
		if(guideConf.dashboardCtx) t = 1;
	    $( "div[cType=5]" ).droppable({
	        accept: "div[iType=t]",
	        //activeClass: "ui-state-hover",
	        //hoverClass: "ui-state-active",
	        drop: function( event, ui ) {
	    		if(!guideConf.dashboardCtx) divWhere.attr("sel",0).parent().css("background-color","");
				...
				if(guideConf.dashboardCtx) aly.addParam(item);
	    			setTimeout("aly.refresh()",1);
	    		}
	        }
	    });
		...
		if(guideConf.dashboardCtx) {
			aly.cache.doms = {div1:div1,div2:div2,div3:div3,div4:div4,divs:divs};
		}else{
...
//			buts.find('[order="'+field.order+'"]').css('visibility','visible');
		...
		,init : function(f) {
			...
			var set = null;
			if (conf.type == 1) {
				var fdt = field.dataType;
				fdt = get5TypesByDTType(field.dataType);
				set = field._fieldType == 'aggr'?(fdt==1?sets[1]:sets[4]):(field._fieldType.indexOf('nalyse')>=0?sets[2]:sets[0]);
				if(field._fieldType == 'aggr' && conf.tops.length == 0 && conf.lefts.length != 0){
					set.push(1);
					set.push(2);
					set.push(3);
	...
	,refreshReport : function(confName, noCalc, noRefreshDialog, drillRpx) {
		...
		for (var i=0; i<rqAnalyse.rpxs.length; i++) {
			...
			if (dlg == null) {
				if(guideConf.dashboardCtx) controlUtil.dashboardParse(currArea);
				...
				if(!guideConf.dashboardCtx) dlg.show();
					...
					$.ajax({
						...
						success: function(data){
							...
							if (data == 1) {
								...
								var u = guideConf.reportPage+"?reportId="+confi.reportId+"&guideDir="+guideConf.guideDir+"&isOlap=no"+"&confName="+encodeURIComponent(confi.name)+"&finish=1&rpxFile="+encodeURIComponent(n);
								if(guideConf.dashboardCtx) u += "&params"+encodeURIComponent(params);
								$('iframe[confName="'+confi.name+'"]').attr('src', u);
								return;
							}
						}
					});
	...
	,editCalcField : function(fieldName, callback) {
		...
		var dlg = art.dialog({
		    ...
		    content: '<div style="margin:10px;">'+resources.guide.js164+'：<input type="text" id="calcFieldName" value="'+(field==null?"":field.name)+'" style="width:120px;"></div>'
		    	...
		    	+'<div style="margin:10px;">'+resources.guide.js166+'：<input placeholder="'+resources.guide.js167+'" type="text" id="calcFieldExp" value="" style="width:370px;"></div>'
	...
	,generateReport : function(conf) {
		...
		var success = function(data){
			...
			var u = guideConf.reportPage+"?reportId="+conf.reportId+"&guideDir="+guideConf.guideDir+"&isOlap=no"+"&confName="+encodeURIComponent(conf.name)+"&finish="+finish;
			if(guideConf.dashboardCtx) u += "&params"+encodeURIComponent(aly.params);
			$('iframe[confName="'+conf.name+'"]').attr('src', u);
		}
		...
		var olapRpx = function() {
			...
			$.ajax({
				...
				async : true,
				url: contextPath + "/DLServletAjax?d="+new Date().getTime(),
				data: {action:2,oper:'genReport',reportId:conf.reportId,title:conf.name,rpt:JSON.stringify(rpt).replaceAll('"','<d_q>'),lefts:rpt.lefts,tops:rpt.tops,maxSize:guideConf.maxReportSize,resultRpxPrefixOnServer:guideConf.resultRpxPrefixOnServer,onlyRefreshDs:guideConf.onlyRefreshDs},
				//confs:[{type:1自定义分析报表/2模板报表,name:'报表名称',reportId:'',template:'',show:0/1,isRowData:1,lefts:[{name:'',item:itemId,use:1,order:0无序/1升序/2降序}],tops:[],fields:[],wheres:[{item:itemId}]}...] 调序、排序、改名
				success: success
			});
		}
	...
	,getGlmdTypes : function(conf){
		var fs = rpxu.getRpxFields(conf);
		var gSrcTypes = "";
		if (conf.tops.length!=0 || conf.lefts.length!=0 || conf.fields.length!=0) {
			if (conf.type == 1 || conf.type == 2) {
				for (var j=0; j<fs.length; j++) {
					var srcn = fs[j].srcName;
					//if (existItems.indexOf(e)>=0) continue;
					if(fs[j].aggr && fs[j].aggr != "" && fs[j]._fieldType == "aggr") {
						srcn = fs[j].name;
						var numType = 1;
						if(fs[j].aggr == "avg") numType = 5;
						if(fs[j].aggr == "sum") numType = fs[j].dataType;
						if(fs[j].aggr == "count") numType = 3;
						gSrcTypes += "," + srcn + ":" + numType;
					}
					else gSrcTypes += "," + srcn + ":" + fs[j].dataType;
				}
			}
		}
		if(gSrcTypes.length > 0) gSrcTypes = gSrcTypes.split(',');
		else gSrcTypes = new Array();
		var glmdDataType = '[';
		for(var t = 0; t < gSrcTypes.length; t++){
			if(gSrcTypes[t].length > 0){
				if(glmdDataType.length > 1) glmdDataType += ',';
				var st = gSrcTypes[t].split(':');//dingdan : 2
				glmdDataType += '{';
				glmdDataType += ('name:'+st[0]);
				glmdDataType += ',';
				glmdDataType += ('type:'+st[1]);
				glmdDataType += '}';
			}
		}
		glmdDataType += ']';
	...
	,queryData : function(conf) {
		...
		var dataSet = aly.getDataSet(conf.dataSet);
		if (dataSet.type == 6){
			var dql = guideConf.dashboardCtx ? rpxu.getDqlSegments4dbd(conf,aly.params) : rpxu.getDqlSegments(conf);
			if (dql == null) {
				return;
			}
			$('img[confNameLoading="'+conf.name+'"]').css('visibility','visible');
			var reportId = conf.reportId;
			var operStr = 'queryDqlData';
			var glmdDataType = aly.getGlmdTypes(conf);
			if(isGlmd) operStr = 'queryGroupDqlData';
			$.ajax({
				type: 'POST',
				async : false,
				url: contextPath + "/DLServletAjax?d="+new Date().getTime(),
				data: {action:2,oper:operStr,reportId:conf.reportId,dqlSegments:dql,dataSource:dataSet.dataSource,maxDataSize:guideConf.maxDataSize,filter:guideConf.filter,glmdDataType:glmdDataType},
				success: function(data){
					if (data.indexOf('error:')==0) {
						alert(data.substring(6));
						return;
	...
	,calcDfxData : function(dataSetField, conf, callback) {
		...
		function f() {
			$.ajax({
				...
				async : false,
				url: contextPath + "/DLServletAjax?d="+new Date().getTime(),
				data: {action:2,isQuery:guideConf.isQuery == null ? 0 : guideConf.isQuery,isGlmd:isGlmd,oper:'calc',reportId:reportId,dataId:dataId,cacheType:cacheType,calcs:calcs,filters:filters,fields:fields,aggrFieldFilters:aggrFieldFilters,resultExp:resultExp,types:r.types,srcTypes:r.srcTypes,dataFileType:guideConf.dataFileType,maxDataSize:guideConf.maxDataSize},
				success: function(data){
					if (data.indexOf('error:')==0) {
						alert(data.substring(6));
						return;
...
function reportCellDrill(reportConfName, topValues, leftValues, fieldValues, detail) {
	...
	for (var i=0; i<topValues.length; i++)
	{
		...
		if (dataSet.type == 6 || dataSet.type == 7) {
			var whereConf = [{"level":1,"fieldInfo":transWhereInfo(conf2.tops[i].src,null,false),"oper":resources.guide.js25,"values":""+defaultEdit.getEditStyleValueByDisp(conf2.tops[i].srcEdit,topValues[i]),"disp":""}];
	...
	for (var i=0; i<leftValues.length; i++)
	{
		...
		if (dataSet.type == 6 || dataSet.type == 7) {
			var whereConf = [{"level":1,"fieldInfo":transWhereInfo(conf2.lefts[i].src,null,false),"oper":resources.guide.js25,"values":""+defaultEdit.getEditStyleValueByDisp(conf2.lefts[i].srcEdit,leftValues[i]),"disp":""}]
...
function saveFile(content,type,needDataPath,oldDataPath) {
	...
	var dlg = art.dialog({
	    ...
	    content: '<input id="saveGrpxName" type="text" value="'+guideConf.defaultSaveName+'" placeholder="'+resources.guide.js173+'" style="width:150px;margin:5px 22px;">'
	    	+'<div><input id="saveGrpxChk1" type="checkbox" checked style="cursor:pointer;vertical-align:-2px;">'+resources.guide.js174+'</div>'
	    	+'<div><input id="saveGrpxChk2" type="checkbox" style="cursor:pointer;"><input id="saveGrpxPath1" type="text" placeholder="'+resources.guide.js175+'" style="width:350px;margin:10px 2px;"></div>'
...
function openDataFile(){
	openFile("fileData",function(content, file){
		...
		form.append('<input id="db" type="hidden" name="dataSource">');
		form.find('#params').val("f="+srcf);
		form.find('#db').val(guideConf.dataSource);
		$('body').append(form);
		form[0].submit();
	});
}
