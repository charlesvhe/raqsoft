...
function setConfig(isEnd, conf) {
	...
	if (confTmp != '') {
		lmd = JSON.parse(confTmp.replaceAll('<d__q>','\\"').replaceAll('<d_q>','"'));
		lmd.editStyles = [];
		/*
...
function generateFields(tables,onlyPks) {
	...
	var showT = ($('#fieldShowStyle').val() == '分类-数据项' || guideConf.view=='analyse');
	if (!showT) {
		...
		for (var i=0; i<ts.length; i++) {
			...
			if (currTable.fks) {
				for (var j=0; j<currTable.fks.length; j++) {
					...
					var n = treeObj.addNodes(node, [{id:currIndex, isParent:true, pId:pId, name:fa, open:false, icon:consts.relaPath+"/js/ztree/css/zTreeStyle/img/diy/4.png",infos:infos1,destTable:currTable.fks[j].destTable,tName:ti,fName:fj,load:false,font:font1}], true);
			...
			if (!(fName && fName!='')) {
				if (currTable.subTables && guideConf.showSubTable=='yes') {
...
function generateDimFields(relaDims) {
	setTimeout(function(){
		...
		list.html('<div style="padding-top:0px;height:20px;padding:8px 0 5px 18px;background-color:#F0F0F0;">分组维度</div>');
		//color:#000;;
...
function addDim(container, dim, alias) {
	...
	var curr = $('<div style="padding:0px;margin:0px;border:0px;" class="item"><h3 id="dimItem" style="outline:0px;border:1px solid ' + consts.color12 + ';border-bottom:0;border-radius:0px;height:26px;background-image:url('+consts.relaPath + consts.imgFolder + consts.img26 + ');color:' + consts.color23 + ';margin:0px;margin-top:0px;'+bold+'" dim="' + dim + '" title="' + dim + '" t="' + dimObj.table + '"><div style="padding:4px 0;"><span><img src="'+consts.relaPath + consts.imgFolder + fImg + '"></span>' + exp + '<div style="float:right;padding:0 2px 0 0;" id="openImg">' + openImg + '</div>' + '</div></h3>' + conts + '</div>');
...
function submitQuery() {
	...
	if (domUtils.toString() == oldConf) {
		...
		//
		rpxData = {srcDs:{fields:domUtils.getAllFields(),resource:{type:2,dataId:guideConf.dataFolderOnServer+"/temp/q"+new Date().getTime()+(guideConf.dataFileType.toLowerCase()=="binary"?".bin":".txt"),rpxData:null,dataSource:guideConf.dataSource,dqlConf:domInfos,ql:dql,dfxFile:'',dfxScript:'',dfxParams:'',inputFiles:'',currTable:'',tableNames:''}},items:[],confs:[],editStyles:[],maxId:0};
		grpx = rpx.toString();
	}
...
$(document).ready(function(){
	...
	if ("source" == guideConf.view) {
		...
		artDialogMaxTop = 50;
		rpx.prepare();
	} else if ("analyse" == guideConf.view) {
		$("#dqlQueryBut").css('display',lmd==null?'none':'block');
		...
		//$(window).resize(adjustFloats);
		$("#analyseConf1").draggable({ handle: "#analyseConf2" });
		$("#analyseConf2").css('opacity', 0.8);
		var ac = $("#analyseConf");
		ac.css('opacity', 1);
		$("#analyseConf2 img").click(function(){
			//alert(ac.height());
			if (ac.height() == 0){
...
function moreQuery(type,dataSource,ql,dfxFile,dfxParams1,dfxScript,dfxParams2,inputFiles){
	...
	var selDom4 = getSelectDom(ddss, ddss, guideConf.dataSource);
	selDom4.css({'background-color':'#41455A','border':'0','color':'#bbb','padding':'2px','height':'27px','margin-top':'9px'}).attr('title','').change(function(){
	});
	...
	selDom3.css({'background-color':'#41455A','border':'0','color':'#bbb','padding':'2px','height':'27px','margin-top':'9px'}).attr('title','').change(function(){
	});
	...
	selDom2.css({'background-color':'#41455A','border':'0','color':'#bbb','padding':'2px','height':'27px','margin-top':'9px'}).attr('title','').change(function(){
...
function dqlQueryInReport(){
	zIndexBak = artDialog.defaults.zIndex;
	var dlg = art.dialog({
		id : dialogCount++,
		title : 'DQL查询',
	    content:'' 
	    	+'<div id="miniDqlQuery"></div>'
	    ,ok : function() {
			var dql = miniDqlFrame.generateSql(miniDqlFrame.domInfos);
			if (!dql) return false;

			var grpx = null;
			if (miniDqlFrame.domUtils.toString() == oldConf) {
				grpx = grpxStr;
			} else {
				//
				rpxData = {srcDs:{fields:miniDqlFrame.domUtils.getAllFields(),resource:{type:2,dataId:guideConf.dataFolderOnServer+"/temp/q"+new Date().getTime()+(guideConf.dataFileType.toLowerCase()=="binary"?".bin":".txt"),rpxData:null,dataSource:guideConf.dataSource,dqlConf:miniDqlFrame.domInfos,ql:dql,dfxFile:'',dfxScript:'',dfxParams:'',inputFiles:'',currTable:'',tableNames:''}},items:[],confs:[],editStyles:[],maxId:0};
				grpx = rpx.toString();
			}
			alert(grpx);
			...
			form.append('<input type="hidden" name="dataSource" value="'+guideConf.dataSource+'">');
			form.append('<input type="hidden" name="fixedTable" value="'+guideConf.fixedTable+'">');
			form.append('<input type="hidden" name="grpx" value="'+grpx+'">');
			$('body').append(form);
			form[0].submit();
	    }
	...
	$("#miniDqlQuery").html('<iframe name="miniDqlFrame" id="miniDqlFrame" style="border:0;width:100%;height:100%;"></iframe>');
	var form = $('<form method="post" accept-charset="UTF-8"></form>');
	form.attr('action',guideConf.grpxSourcePage);
	form.attr('target', 'miniDqlFrame');
	form.append('<input type="hidden" name="view" value="'+('source')+'">');
	form.append('<input type="hidden" name="dataSource" value="'+guideConf.dataSource+'">');
	form.append('<input type="hidden" name="fixedTable" value="'+guideConf.fixedTable+'">');
	form.append('<input type="hidden" name="showToolBar" value="no">');
	form.append('<input type="hidden" name="showSubTable" value="no">');
	form.append('<input type="hidden" name="grpx" value="'+grpxStr+'">');
	$('body').append(form);
	form[0].submit();
