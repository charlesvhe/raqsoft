...
function DqlQuery() {
			...
			for (var i=0; i<dct.dimItemList.length; i++) {
				...
				try {
					if (di.codeCol) {
						...
						if (di.dispCol) d.item.disp = mdUtils.getFieldByDisp(di.dispCol.split(".")[0],di.dispCol.split(".")[1],true).name;
	...
	obj.setConfStr = function(str) {
		if (str && str != '') {
			str = str.replaceAll('<d__q>','\\"').replaceAll('<d_q>','"');
			try{
...
//			}
			...
			for (var i=0; i<srcs.length; i++) {
				...
				if (oi.subTable) {
					lastTables.push(oi.subTable);
					var oi1 = domUtils.getInfosObj(oi.subTable);
					if (allTables.indexOf(oi.subTable) == -1)allTables.push(oi.subTable);
		...
		getFieldInfos : function(parent, fields, levelCount, table, field, containPk, templevelCount, drill) {
			...
			if (levelCount > 1) {//guideConf.showSubTable
			...
			*/
			...
			for (var i=0; i<fields.length; i++) {
				...
				sgts.fields.push({
					...
					,aggr:(iObj.subTable == null || iObj.subTable == '' ) ? iObj.aggr : iObj.subTableAggr
...
function EditStyle() {
	...
	var obj2 = {
		...
		getEditStyle : function(name){ //通过名字获得编辑风格
			if (!name) return null;
			if(mdUtils == null) return null;
			var dims = mdUtils.getDims(null,true);
			for(var j = 0; j < dims.length; j++){
				if(dims[j].name == name) name = dims[j].dispName;
			}
		...
		getEditExp4Rpx : function(edit,isRealNum) {
			if(edit.indexOf('diy=') == 0){
				return edit.substr(4);//自定义的
			}
			//alert(edit);
		...
		registerEsEvent : function(es, doms, exp, useTreeDisp){
			...
			var dims = mdUtils.getDims(null,true);
			for(var j = 0; j < dims.length; j++){
				if(dims[j].name == es) es = dims[j].dispName;
			}
			for (var i=0; i<editStyles.length; i++) {
...
function Olap(){
	...
	obj.setConfStr = function(str) {
		var a = null;
		try{
			a = eval("("+ str.replaceAll('<d__q>','\\"').replaceAll("<d_q>",'"').replaceAll("\r\n",'').replaceAll("\n",'')+")");
		}catch(e){
			alert('读取有误：'+str);
		}
		for (var i=0; i<a.dataSets.length; i++) {
			...
			if(guideConf.alwaysRefreshCache){//保存但是打开时候用guideConf参数控制是否用旧的dataId，是否刷新(alwaysRefreshCache)
				dsi.dataId = "";
				console.log('clean dataId');
			}
			if (dsi.fields) {
	...
	obj.getConfStr = function() {
		...
		for (var i=0; i<a.dataSets.length; i++) {
			var dsi = a.dataSets[i];
			//保存olap的时候不再保存dataSets中的dataId，以在打开的时候刷新缓存?改为保存但是打开时候用guideConf参数控制是否用旧的dataId，是否刷新(alwaysRefreshCache)
			//dsi.dataId = "";
			...
			if(
				...
				a.dataSets = a.dataSets.slice(0, i).concat(a.dataSets.slice(parseInt(i, 10) + 1));
			}
			for (var k=0; k<a.dataSets.length; k++) {
				var dsi = a.dataSets[k];
				dsi.ql = dsi.ql.replaceAll('"',"<d__q>");
				dsi.ql = dsi.ql.replaceAll("\n",'<_n_>').replaceAll("\t",'<_t_>');;
				dsi.dfxParams = dsi.dfxParams.replaceAll('"',"<d__q>");
		...
		for (var j=0; j<a.rpxs.length; j++) {//新打开的olap不要缓存的rid
			a.rpxs[j].reportId = "";
			var rj = a.rpxs[j];
			for(var k = 0; k < rj.fields.length; k++){
				var fieldk = rj.fields[k];
				if(fieldk.diyEdit) fieldk.diyEdit = fieldk.diyEdit.replaceAll('"',"<d__q>");
			}
			for(var k1 = 0; k1 < rj.lefts.length; k1++){
				var left = rj.lefts[k1];
				if(left.diyEdit) left.diyEdit = left.diyEdit.replaceAll('"',"<d__q>");
			}
			for(var k2 = 0; k2 < rj.tops.length; k2++){
				var top = rj.tops[k2];
				if(top.diyEdit) top.diyEdit = top.diyEdit.replaceAll('"',"<d__q>");
			}
		}
		...
		,getDqlSegments : function(conf) {
			...
			if (conf.fields.length==0) {
				...
				for (var i=0; i<conf.tops.length; i++) {
					...
					if(fieldi && fieldi.calc) expNoTableNoAggr = tops[i].calc;
				...
				for (var i=0; i<conf.lefts.length; i++) {
					...
					var expNoTableNoAggr = jObj.expNoTableNoAggr;
					if(fieldi && fieldi.calc) expNoTableNoAggr = lefts[i].calc;
					tj.bys.push(expNoTableNoAggr);
				}
			...
			for (var i=0; i<conf.tops.length; i++)
			{
				...
				if (topi.relas.length>0) {
					for (var j=0; j<topi.relas.length; j++) {
						...
						var expNoTableNoAggr = jObj.expNoTableNoAggr;
						if(fieldi && fieldi.calc) expNoTableNoAggr = topi.calc;
						tj.bys.push(expNoTableNoAggr);
					}
				}
			...
			for (var i=0; i<conf.lefts.length; i++)
			{
				...
				if (lefti.relas.length>0) {
					for (var j=0; j<lefti.relas.length; j++) {
						...
						var expNoTableNoAggr = jObj.expNoTableNoAggr;
						if(fieldi && fieldi.calc) expNoTableNoAggr = lefti.calc;
						tj.bys.push(expNoTableNoAggr);
					}
				}
				//bys.push("T1"+subTable+"."+iObj.expNoTableNoAggr);
			...
			for (var i=0; i<conf.fields.length; i++)
			{
				...
				var iObj = dqlQuery.confUtils.getInfosObj(fieldi.autoSrc == null ? fieldi.src : fieldi.autoSrc, fieldi.calc);//2020.02.20上级的上级问题, || fieldi.autoSrc != fieldi.src
				...
				var isCalc = false;
				if(fieldi && fieldi.calc) {
					expNoTableNoAggr = fieldi.calc;
					isCalc = true;
				}
...
//			return dql;
		...
		,getDfxExps : function(exceptWhereItem, rpx) {
			...
			if (conf.where.conf != null && conf.where.conf.length>0) {
				if (filter != "") filter += " && ";
				filter += "("+whereUtils.getExp(conf.where.conf, "", 1, 2,conf)+")";
			}
			...
			for (var j=0; j<fs.length; j++) {
				...
				if(fs[j].aggr != ""){
					if (aggrFieldFilter != "") aggrFieldFilter += " && ";
					aggrFieldFilter += "("+whereUtils.getExp(fs[j].where.conf, "", 1, 2,fs[j])+")";
				}else{
					if (filter != "") filter += " && ";
					filter += "("+whereUtils.getExp(fs[j].where.conf, "", 1, 2,fs[j])+")";
				}
			}
			if (calc == '') calc = 'no';
			if (filter == '') filter = 'no';
			...
			if (!exceptWhereItem) {
				aggrFieldFilters = aggrFieldFilter;
			}
			if (!exceptWhereItem) {
				//var resultExp = "groups(说明:A;count(a2):B;1)";
			...
			return {calcs:calcs,filters:filters,fields:fields,aggrFieldFilters:aggrFieldFilters,resultExp:resultExp,dataId:dataId,types:types,srcTypes:srcTypes};
		}
		...
		,checkFieldName : function(rpx, f){
			...
			if (dataSet.type == 6){
				try {
					var alertBack = alert;
					alert = function(){};
					var iObj = dqlQuery.confUtils.getInfosObj(f.src,f.calc);
					...
					if (f._parentType != 'field') {
						...
						if (invalidTable != "") {
							f._status = "No dim define, maybe lmd has changed or no dql license! ";
		...
		if (conf.tops.length==0&&conf.lefts.length==0) {
			...
			if (conf.isRowData) {
				...
				for (var i=0; i<fields.length; i++) {
					...
					rpt.cells.push({row:2,col:i+1,row2:2,col2:i+1,format:fields[i].format,value:'',valueExp:getExps(conf,fields[i],i==0?3:5,i==0?orders:''),extend:16,leftMain:'',topMain:'',tip:4,adjustSizeMode:48,textWrap:1,hAlign:zhibiao[0].hAlign,color:zhibiao[0].color,backColor:zhibiao[0].backColor,dispExp:defaultEdit.getEditExp4Rpx(fields[i].diyEdit == null?fields[i].srcEdit:"diy="+fields[i].diyEdit,fields[i].dataType==1),formatExp:formatExp});
			...
			if (conf.isRowData) {
				...
				for (var i=0; i<lefts.length; i++) {
					if (showZongji) {
						rpt.cells.push({row:firstAggrRow,col:i+1,row2:firstAggrRow+(lefts.length-i-1),col2:i+1,format:conf.lefts[i].format,value:'',valueExp:getExps(conf,conf.lefts[i],2,null,null,null,null,null,fieldOrder),extend:18,leftMain:'',topMain:'',tip:1,adjustSizeMode:48,textWrap:1,hAlign:fenzu.hAlign,color:fenzu.color,backColor:fenzu.backColor,dispExp:defaultEdit.getEditExp4Rpx(conf.lefts[i].diyEdit == null?conf.lefts[i].srcEdit:"diy="+conf.lefts[i].diyEdit,conf.lefts[i].dataType==1),formatExp:formatExp});
						cellNames.push(lefts[i]);
						cellRefs.push(cellCols[i+1]+firstAggrRow);
						rpt.cells.push({row:firstAggrRow+(lefts.length-i),col:i+1,row2:firstAggrRow+(lefts.length-i),col2:lefts.length,format:conf.lefts[i].format,value:resources.guide.js228,valueExp:'',extend:18,leftMain:'',topMain:'',tip:10,adjustSizeMode:48,textWrap:1,hAlign:zongji.hAlign,color:zongji.color,backColor:zongji.backColor,dispExp:'',formatExp:''});
					} else {
				...
				for (var i=maxTop; i>=0; i--) {
					...
					for (var j=0; j<fields.length; j++) {
						...
						var cell = {row:firstAggrRow,col:currCol,row2:firstAggrRow,col2:currCol,format:fields[j].format,value:'',valueExp:exp,extend:16,leftMain:leftMain,topMain:topMain,tip:3,adjustSizeMode:48,textWrap:1,hAlign:zhibiaoj.hAlign,color:zhibiaoj.color,backColor:zhibiaoj.backColor,dispExp:fields[j].diyEdit==null?'':fields[j].diyEdit,formatExp:formatExp};
					...
					if (i>0) {
						rpt.cells.push({row:i,col:lefts.length+1,row2:i,col2:fields.length==0?currCol+1:currCol,format:conf.tops[i-1].format,value:'',valueExp:getExps(conf,conf.tops[i-1],2),extend:19,leftMain:'',topMain:'',tip:2,adjustSizeMode:48,textWrap:1,hAlign:fenzu.hAlign,color:fenzu.color,backColor:fenzu.backColor,dispExp:defaultEdit.getEditExp4Rpx(conf.tops[i-1].diyEdit == null?conf.tops[i-1].srcEdit:"diy="+conf.tops[i-1].diyEdit,conf.tops[i-1].dataType==1),formatExp:formatExp});
				...
				for (var i=0; i<finalFields.length; i++) {
					...
					//2019.11.14
					if(guideConf.infinity != null && guideConf.infinity.length > 0){ 
						if(finalCells[i].dispExp == null || finalCells[i].dispExp == "") 
//							finalCells[i].dispExp = "if((1/0)==double(value()),\""+guideConf.infinity+"\",string(value()))";
//						else
//							finalCells[i].dispExp = "if((1/0)==double("+finalCells[i].dispExp+"),\""+guideConf.infinity+"\",string("+finalCells[i].dispExp+"))";
						
							finalCells[i].dispExp = "if((1/0)==double(if(ifnumber(value()),value(),0)),\""+guideConf.infinity+"\",value())";
						else
							finalCells[i].dispExp = "if((1/0)==double(if(ifnumber("+finalCells[i].dispExp+"),"+finalCells[i].dispExp+",0)),\""+guideConf.infinity+"\","+finalCells[i].dispExp+")";
					}
				}
			//纵向显示数据
			} else {
				...
				for (var i=maxLeft; i>=0; i--) {
					...
					if (i>0) {
						rpt.cells.push({row:conf.tops.length+1,col:i,row2:fields.length==0?currRow+1:currRow,col2:i,format:conf.lefts[i-1].format,value:'',valueExp:getExps(conf,conf.lefts[i-1],2),extend:18,leftMain:'',topMain:'',tip:1,adjustSizeMode:48,textWrap:1,hAlign:fenzu.hAlign,color:fenzu.color,backColor:fenzu.backColor,dispExp:defaultEdit.getEditExp4Rpx(conf.lefts[i-1].diyEdit == null?conf.lefts[i-1].srcEdit:"diy="+conf.lefts[i-1].diyEdit,conf.lefts[i-1].dataType==1),formatExp:formatExp});
		...
		//没有生成表头，行类型设置重复，2019.9.3
		for (var i=1; i<=rpt.rowCount; i++) {
			if(rpt.rows[i-1].row == i) break;
