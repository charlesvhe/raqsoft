...
String norefreshfilter = request.getParameter( "norefreshfilter" );
if (norefreshfilter == null) norefreshfilter = "";
ArrayList filters = new ArrayList();
ArrayList oldFilters = (ArrayList) session.getAttribute("_raqsoft_filters_");
...
if(oldFilters != null && oldFilters.size()>0 && (vsb == null || vsb.length() == 0) && norefreshfilter.equals("yes")){
	...
	f.getParamValues().put("编号","6");//设置过滤条件里的参数值（宏值）
}
%>
...
guideConf.dataFileType = "<%=dataFileType%>";
	...
	*/
	...
	var sqlDatasets = [
		{
			sqlId:"s1"
			,dataSource:"test" //如果服务器管理这些，js里就不用定义
			,sql:"select * from test.test0212" //如果服务器管理这些，js里就不用定义
			,fields:null
		}
	]
	...
	guideConf.queryPage = 'raqsoft/guide/jsp/qyx.jsp';
	guideConf.scanRow=100000;
	//guideConf.maxDataSize = 10;
	guideConf.dimDataOnServer = "WEB-INF/files/data/temp/dimData.json";
	guideConf.infinity="-";
