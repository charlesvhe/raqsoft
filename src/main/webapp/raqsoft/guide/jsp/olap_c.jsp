...
System.out.println("Context.getJspCharset : " + com.raqsoft.report.usermodel.Context.getJspCharset());
...
String resetLmd = "yes";
resetLmd = request.getParameter( "resetLmd" );
...
//dct = "WEB-INF/files/dql/demo.gdct";
//vsb = "WEB-INF/files/dql/demo.gvsb";
...
if (ds1.length()>0) {
	DQLTableFilter f = new DQLTableFilter("default",ds1);
	filters.add(f);
...
<script>
	...
	*/
	...
	var sqlDatasets = [
		{
			...
			,sql:"SELECT T0.(VIP客户) VIP客户 ,T0.(VIP级别) VIP级别 ,T0.(VIP折扣率) VIP折扣率 ,T0.(VIP结束时间) VIP结束时间 ,T0.(VIP起始时间) VIP起始时间 ,T0.(客户) 客户 ,T0.(客户名称) 客户名称 ,T0.(联系人姓名) 联系人姓名 ,T0.(联系人职务) 联系人职务 ,T0.(市) 市 FROM VIP客户 T0 " //如果服务器管理这些，js里就不用定义
			,fields:null
		}
	]
	...
	guideConf.queryPage = 'raqsoft/guide/jsp/qyx_c.jsp';
	guideConf.scanRow=100000;
	...
	//guideConf.onlyRefreshDs = "yes";
	try{
		if(noGlmdFile) alert("No glmd file opened");
	}catch(e){}
</script>
