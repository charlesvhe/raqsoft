			...
			callback: {
				...
				,beforeDrag: function(treeId, treeNodes) {
					...
					*/
					...
					if (aly.cache.doms.div4){
						aly.cache.doms.div4.bind('mouseover',function(){
							aly.cache.doms.div4.find('#placeHolders').css('display','block');
						}).bind('mouseout',function(){
							aly.cache.doms.div4.find('#placeHolders').css('display','none');
						});
					}
				...
				,onDrop: function(e, treeId, treeNodes, targetNode, moveType) {
					...
					//div4删除
					if (aly.cache.doms.div5){
						aly.cache.doms.div5.unbind('mouseover').unbind('mouseout');
						//alert(aly.cache.doms.div2.find('#placeHolders').css('display'));
						if (aly.cache.doms.div5.find('#placeHolders').css('display') == 'block'){
							pos = 'params';
						}
						aly.cache.doms.div5.find('#placeHolders').css('display','none');
					}
					...
					for (var z=0; z<ss.length; z++) {
						...
						//if(treeNodes[z] != null && treeNodes[z].calc) newItem.calc = treeNodes[z].calc;
						if (divsi != null) {
...
function makeDraggable(elm,artDialogMaxTop){
	elm.draggable({ handle: "#analyseConf2",stop:function(event, ui ){
		if (parseInt(elm.css("top"))<artDialogMaxTop){
			elm.css('top',artDialogMaxTop+'px');
		}
	}});
}
...
//		makeDraggable(ac1,artDialogMaxTop);
		var aggrModel = $('#aggrModels');
//		makeDraggable(aggrModel,artDialogMaxTop);
//		ac2.css('opacity', 0.8);
//		var ac = $("#analyseConf");
//		ac.css('opacity', 1);
...
//		$('body').css('overflow','auto')
		if (olapStr != '') {
			...
			for (var ii = 0; ii<rqAnalyse.rpxs.length; ii++) {
				if (!rqAnalyse.rpxs[ii].reportId) rqAnalyse.rpxs[ii].reportId = "rid"+ getTime();
				else if ((rqAnalyse.rpxs[ii].reportId+"").indexOf("rid") == -1) rqAnalyse.rpxs[ii].reportId = "rid"+getTime();//dbd function
			}
		} else {
			var type = 6;
		...
		//2019.11.27
		if(guideConf.openDOlap == "false"){
			dbdInit();
		}
		
		analyseApi.udf.modifyOlap();
		olapObj.setConf(rqAnalyse);
		if( !finalView ) parseAggrModels(aggrModel);
		...
		for (var i=0; i<rqAnalyse.dataSets.length; i++){
			...
			if (dsi.type == 6) {
				...
				if (dsi.fields == null) {
					...
					olapObj.server({action:2,oper:'getTableInfo',sqlId:dsi.sqlId,sql:currsd.sql,dataSource:currsd.dataSource,scanRow:guideConf.scanRow,isCursor:guideConf.dataFileType=='text'?2:1},function(data){
							if (data.indexOf('error:')==0) {
...
function manageDataSet(){
	...
	var dsClick = function(){
		...
		currDataSet.html(
			'<div style="margin:5px 0px 10px;"><input id="dataSetName" type="text" style="height:26px;"><input id="saveDsBut" type="button" value="'+resources.guide.js51+'" style="float:right;"></div>'
	    	...
	    	+'<div id="dbQuery"><div style="" id="dataSources"></div><div style=""><textarea id="ql" style="width:540px;height:150px;" placeholder="'+resources.guide.js85+'"></textarea></div><div style=""><input type="text" style="width:300px;height:28px;" dataId="2"><input dataIdBut="2" style="float:right;height:29px;" type="button" value="'+resources.guide.js86+'"></div></div>'//<span id="dsStatus"></span>
	    	+'<div style="display:none;" id="dfxFileQuery"><div style="" id="dfxFile"></div><div style=""><textarea id="dfxParams1" style="width:540px;height:150px;" placeholder="'+resources.guide.js87+'"></textarea></div><div style=""><input type="text" style="width:300px;height:28px;" dataId="3"><input dataIdBut="3" style="float:right;height:29px;margin-right:10px;" type="button" value="'+resources.guide.js86+'"></div></div>'
	    	+'<div style="display:none;" id="dfxScriptQuery"><div style="margin-top:10px;"><textarea id="dfxScript" style="width:540px;height:120px;" placeholder="'+resources.guide.js89+'"></textarea></div><div style=""><textarea placeholder="'+resources.guide.js90+'" id="dfxParams2" style="width:540px;height:50px;"></textarea></div><div style="margin-top:10px;"><input dataId="4" type="text" style="width:300px;height:28px;"><input dataIdBut="4" style="float:right;height:29px;margin-right:10px;" type="button" value="'+resources.guide.js86+'"></div></div>'
	    	+'<div style="display:none;" id="inputQuery"><div style="" id="inputFile"></div><div style=""><textarea id="inputFiles" style="width:540px;height:150px;" placeholder="'+resources.guide.js88+'"></textarea></div><div style=""><input dataId="5" type="text" style="width:300px;height:28px;"><input style="float:right;height:29px;margin-right:10px;" dataIdBut="5" type="button" value="'+resources.guide.js86+'"></div></div>'
		...
		var saveFunc = function(silence){
			...
			$('#dsSelectDiv').find('select').find('option[value='+ds.name+']').attr('value',name).html(name);
			$('#selectDs').find('option[value='+ds.name+']').attr('value',name).html(name);
			ds.name = name;
			if (silence!==true) alert(resources.guide.js99);
			aly.refresh(true,true);
			//遇到切换类型后字段checkconf不正确的问题；跳过check，取消选择div，对数据集重新查询
			var d = currDiv;
	...
	var delClick = function(){
		...
		$(this).parent().remove();
		$('#dsSelectDiv').find('select').find('option[value='+ds.name+']').remove();
		$('#selectDs').find('option[value='+ds.name+']').remove();
	}
	...
	addBut.click(function(){
		...
		//if(aly.getDataSet(n+1) == null) n = n + 1;
		//else {alert("one dataset only");return;}
		...
		$('#dsSelectDiv').find('select').append('<option value="'+n+'">'+n+'</option>');
		$('#selectDs').append('<option value="'+n+'">'+n+'</option>');
...
function parseAggrModels(aggrModel){
	const rpx_img_json = {
			'三维饼形图':'pie_3d',
			'饼形图':'pie',
			'堆积柱形图':'pileupColumns',
	...
	var ul = getModelUl(existRpx.length==0?[""]:existRpx,rpx_img_jsonArr);
	$(aggrModel).find('#analyseConf4').append(ul);
	$(ul).find('li').css("cursor","move").css('margin-top','10px');
	var ds = [];
	for (var i=0; i<rqAnalyse.dataSets.length; i++)
	{
	...
	var width = '160';
	var selDom2 = getSelectDom(ds.length==0?[""]:ds, ds.length==0?[resources.guide.js121]:ds,"",width);
	$('#dsSelectDiv').append(selDom2);
	...
	$('#analyseConf4').find('ul').css("list-style","none").css("margin","10px").css('width',width+"px");
}