...
cleanLastChangeOnMouseDown();
var dsu = olapObj.dataSetUtils;
var rpxu = olapObj.rpxUtils;
var rqAnalyse = null;

function refreshStatus(dataSet,finishFunc,container,doNotShowStatus) {
	if(!doNotShowStatus){
		if (!container) container = $('#dsStatus');
		if (!container[0]) {
			$('#dataHeader').append('<nav style="display:inline;float:right" id="queryStatus"></nav>');
			container = $('#queryStatus');
		}
	}
	var callback = function(data) {
		...
		if (data.over == 1) {
			dataSet.over=1;
			if (data.loadedRow == 0) {
				if(container) container.html(resources.guide.js104.replaceAll("{0}",dataSet.name));
				else alert(resources.guide.js104.replaceAll("{0}",dataSet.name));
				return;
			}
			if (finishFunc) finishFunc();
			if(container) container.html(resources.guide.js105.replaceAll("{0}",dataSet.name).replaceAll("{1}",data.loadedRow));
			else alert(resources.guide.js105.replaceAll("{0}",dataSet.name).replaceAll("{1}",data.loadedRow));
			setTimeout(function(){
				$('#queryStatus').remove();
			},2000);
		} else {
			if (data.loadedRow == 0) {
				if(container) container.html(resources.guide.js106.replaceAll("{0}",dataSet.name));
				//else alert(resources.guide.js106.replaceAll("{0}",dataSet.name));
			} else {
				if(container) container.html(resources.guide.js107.replaceAll("{0}",dataSet.name).replaceAll("{1}",data.loadedRow));
				else alert(resources.guide.js107.replaceAll("{0}",dataSet.name).replaceAll("{1}",data.loadedRow));
			}
...
var aly = {
	...
	,addParam:function(item){
		if(!this.params[item.srcName]){
			this.paramNames.push(item.srcName);
			this.params.push({name:item.srcName,value:"",item:item});
		}
	}
	,getParamItem:function(name){
		for(var p = 0; p < this.params.length; p++){
			if(this.params[p].name == name){
				return this.params[p].item;
			}
		}
		return null;
	}
	,paramDel:function(delparam){
		if(!delparam || delparam.length == 0) delparam = this.currParam;
		for(var p = 0; p < this.params.length; p++){
			if(this.params[p].name == delparam){
				this.params.splice(p,1);
				this.paramNames.splice(p,1);
			}
		}
		this.currParam="";
		$.powerFloat.hide();
	...
	,getItem:function(itemID){
		...
		//var aSrc = dqlQuery.confUtils.autoSrcs([itemID]);
		//var iiObj = dqlQuery.confUtils.getInfosObj(itemID);
		var parent =  dqlQuery.confUtils.getInfosObj(itemID);
		var targetTable = parent.firstTable;
		...
		for (var j=0; j<ts.length; j++) {
			...
			for (var i=0; i<fs.length; i++) {
				if(fs[i].name == itemID){
					infos = targetTable + split_2 + t + split_1 + fs[i].name;
					break;
				}
			}
		}
...
//	,paramSubmit:function(){
//		var inputs = $('#paramValuesForm').find('input[type=text]');
...
//	}
	...
	,set : function(str) {
		var layout = olapObj.setConfStr(str);
		try{
			var flag = alertLayout;
		}catch(e){flag = true;}
		if(layout == null && flag) alert('选择olap文件无布局样式！');
		rqAnalyse = olapObj.conf;
		if(guideConf.openDOlap == true || guideConf.openDOlap == 'true' ){
//			if(layout == null && rqAnalyse.rpxs.length > 0){
//				layout = {
//					areas:[]
//				};
//				for(var n = 0; n < rqAnalyse.rpxs.length; n++){
//					var rpxname = rqAnalyse.rpxs[n].name;
//					layout.areas[n] = {
//						hw: {h: "0px", w: "0px"},
//						id: 0,
//						inDivCell: "",
...
//			}
			dbdInit(layout);
		}else if(guideConf.openDOlap == "new"){
			//切换布局
			$('#contents').html('');
			for(var n = 0; n < rqAnalyse.rpxs.length; n++){
				var rpxname = rqAnalyse.rpxs[n].name;
				if(layout.areas[n].report == rqAnalyse.rpxs[n].name) continue;
				rpxname = controlUtil.checkGetAreaName(rpxname, layout.areas);
				layout.areas[n].report = rpxname;
				rqAnalyse.rpxs[n].name = rpxname;
			}
			dbdInit(layout);
		}
	}
	,toString : function(rqa, dbdMobileFrame) {
		if (rqa) {
			var o1 = new Olap();
			o1.setConf(rqa);
			return o1.getConfStr();
		} else {
			return olapObj.getConfStr(null,dbdMobileFrame,this.dsWhereParams,this.sysparams);
		}
	}
	...
	,showDataSetRpxs : function(dataSet,lastRpxFunc) {
		...
		if (guideConf.showHistoryRpx != 'yes'){
			...
			if (ds.type!=6 && ds.fields == null) {
				olapObj.server({action:2,oper:'getTableInfo',dataId:ds.dataId,dataFileType:guideConf.dataFileType,scanRow:guideConf.scanRow,isCursor:guideConf.dataFileType=='text'?2:1},function(data){
						if (data.indexOf('error:')==0) {
							ds._status = data.substring(6);
							return;
						}
		...
		*/
		for (var i=0; i<rqAnalyse.rpxs.length; i++) {
			...
			//if (rqAnalyse.currRpx == rpx.name) {
				//aly.refresh();
			//} else 
				if (rpx.dialog.open == 1) aly.refreshReport(rpx.name, false, false);
			initRpxCount++;
			lastRpxFunc();
		}
	}
	...
	,checkConf : function(conf, params) {
		rpxu.checkConf(conf, params);
	}
	,refreshAll : function(){
		var cr = rqAnalyse.currRpx;
		for(var rn = 0; rn < rqAnalyse.rpxs.length; rn++){
			rqAnalyse.currRpx = rqAnalyse.rpxs[rn].name;
			aly.refresh(null,null,null,null,function(){
				if(cr == null) rqAnalyse.currRpx= rqAnalyse.rpxs[0].name;
				else rqAnalyse.currRpx = cr;
			});
			redefineFrameHeight(rqAnalyse.rpxs[rn].name);
		}
	}
	,refresh : function(noCalc, noRefreshDialog, drillRpx, noAggr, cb, zongji) {
		...
		for (var i=rqAnalyse.rpxs.length-1; i>=0; i--) {
			...
			var areaDom = $('.singleArea[confname='+confi.name+']');
			var rowNum = $(areaDom).parent().attr("row");
			var colNum = $(areaDom).parent().attr("col");
			
			var rndiv = $("<div style='padding:5px;cursor:pointer;"+sty+"' confName='"+confi.name+"' title='"+rowNum+"_"+colNum+"'></div>");
			rndiv.append("<img style='vertical-align:-2px;cursor: pointer;' lock=1 src='"+contextPath+(guideConf.guideDir+(confi.autoCalc==1?"/img/guide/17.png":"/img/guide/18.png"))+"'><span style='padding:0 20px 0 5px;vertical-align:1px;'>"+confi.name+"</span><img style='float:right;vertical-align:-2px;cursor:pointer;margin:0 5px;' del=1 src='"+contextPath+guideConf.guideDir+"/img/guide/13.png'><img modify=1 style='vertical-align:-2px;cursor:pointer;margin:0 5px;float:right;' src='"+contextPath+guideConf.guideDir+"/img/guide/31.png'>")
			confsDiv.append(rndiv);
			rndiv.click(function(){
				var targetCell = $('.divCell[row='+rowNum+'][col='+colNum+']')[0];
				divTableControl.changeChoosedDiv(targetCell);
			});
		}
...
//		});
		...
		confsDiv.find('img[modify=1]').click(function(){
			...
			var rcs = getLayoutRowAndCol(rpx.name);
			var row = rcs[0];
			var col = rcs[1];
			var dlg = art.dialog({
				id : dialogCount++,
		    ...
		    content: '<div style="margin:'+(rpx.type==1?'20px':'40px 20px')+';"><input type="text" style="width:350px;" id="modifyNameTxt" value="'+cn+'"></div>'
		    	...
		    	+'<div style="margin:20px;margin-top:0;"><input style="width:350px;" type="text" id="locationRow" placeholder="布局行" value="'+row+'"></div>'
		    	+'<div style="margin:20px;margin-top:0;"><input style="width:350px;" type="text" id="locationCol" placeholder="布局列" value="'+col+'"></div>'
		    ...
		    ,ok : function() {
		    	...
		    	if (cn == n && cw == rpx.colWidths && rh == rpx.rowHeights) {
//		    		return true;
		    	}else{
		    		...
		    		setTimeout(function(){
		    			var currAreaId = controlUtil.changeInfo(cn,n);
		    			currArea = $('#'+currAreaId);
		    			rqAnalyse.currRpx = n;
				...
				var newLocationRow = $('#locationRow').val();
				var newLocationCol = $('#locationCol').val();
				if(newLocationRow != row || newLocationCol != col){
					//移动report到div
					divTableControl.moveReport(null,null,$('.divCell[row='+row+'][col='+col+']').find('.singleArea').attr('id'),newLocationRow, newLocationCol);
				}
		    	return true;
		    }
 		    ,cancel : function() {
		...
		addReport.click(function(){
			//addArea();原有的area放到divcell里
			if(currDiv != null) {
				var sArea = $(currDiv).find('.singleArea');
				if(sArea == null || sArea.length == 0){
					addAreaIntoDivCell($(currDiv).attr('id'));
				}
			}else{
				...
				for(var x = 0; x < divCells.length; x++){
					...
					if(sArea == null || sArea.length == 0){
						addAreaIntoDivCell($(divCells[x]).attr('id'));
						break;
					}
				}
			}
...
//			$('#addReportDs').append(selDom2);
		...
		$(confsTitle).append(addReport).append('<a style="float:right;margin-top:5px;margin-right:10px" href="javascript:showAggrModels();"><img src="../img/aggrimg/charts_pie.png"/></a>').append('<div style="clear:both;"></div>');
		$(t3tds[2]).css('border-right','1px solid #E4E4E4').append(t2);
		
		var conf = aly.getCurrRpx();
		if (conf == null) return; 
		analyseApi.event.changeReport();
		if(!noRefreshDialog) aly.checkConf(conf);
		...
		items = items.length == 1 ? items : $("<div id='items' style='width:220px;overflow: auto;max-height: 400px;'></div>");
		//if (contentDiv.length == 0) 
		contentDiv =  contentDiv.length == 1 ? contentDiv : $("<div id='contentDiv' style='border:0;padding:0px;width:220px;height:400px;overflow:auto;' class='ztree'></div>");
		//$(t2tds[0]).append(aggrs);
		//t2tds[0]是数据集字段列表，可能是树状也可能是字段
		//在此之上加一个参数控制入口
		var paramSetBut = $("<button style='width:220px;border-width:1px;border-color:black;border-style:dashed;cursor:pointer;padding:5px;margin:0 10px 0 0px;'>"+"数据集参数"+"</button>");
		paramSetBut.click(function(){
			var inFunc = function() {
				var filter1 = "";
...
//					}
					aly.dsWhereParams = cache.where1.wheres;
...
//					}
				...
				whereUtils.refresh(fields, initField, JSON.parse(JSON.stringify(dataSet.params)), true);//JSON.parse(JSON.stringify(conf.where.conf)));
			}
			analyseApi.complexWhereFunc(inFunc);
		});
		
		//$(t2tds[0]).append(paramSetBut);
...
//		$(area).unbind("click");
//		$(area).click(function(){
//			var cn = $(this).attr('confName');
//	    		rqAnalyse.currRpx = cn;
//			aly.refresh(true);
//		});
//		$(area).click(function(){rqAnalyse.currRpx = $(this).attr('confName');divTableControl.changeChoosedDiv($(area).parent()[0])});
		$('#gridTable').html('');
...
//			div4 = $(tds[7]).find('div[cType]');//param布局位置错误
			...
			//params放弃的单元格
			if(aly.paramNames && aly.paramNames.length > 0){
//				var form = $('<form id="paramValuesForm"></form>');
//				$('#paramValuesFormTd').html("").append(form);
				for (var i=0; i<aly.params.length; i++) {
					var paramsi = aly.params[i].name;
					var paramValue = aly.params[i].value;
//					form.append('<input name="' + paramsi + '" type="text" value="'+(paramValue == null || paramValue == '' ? '':paramValue)+'"/>')
					var color = '';//paramsi._status == ''?'':'gray';
//					var divi = $("<div title='"+ aly.params[i].item._status+"' style='color:"+color+";' cfName='"+paramsi+"' iType=6 idx='"+i+"'>"+paramsi+"</div>");
//					div4.append(divi);//param布局位置错误
					var orderImg = $('<img paramItemName="'+paramsi+'" style="color:'+color+';vertical-align:-3px;cursor:pointer;margin:0px;" src="'+contextPath+guideConf.guideDir+'/img/guide/21.png">');
					divi.append(orderImg);
//					form.append('<input type=\"button\" onclick=\'aly.paramDel("'+paramsi+'\");\' value=\"删除\"/>');
//					form.append('<input type=\"button\" onclick=\'aly.paramSubmit();\' value=\"刷新\"/>');
				}
//				if(aly.paramNames.length > 0){
//					form.append('<input type="button" onclick="refreshParams();" value="刷新"/>');
//				}
			}
//			else{
//				$('#paramValuesFormTd').html("param values");
//			}
			//param布局位置错误
//			if (!aly.paramNames || aly.paramNames.length == 0) div4.append("<div id='confHints' style='height:30px;text-align: center; margin:3px;padding:3px;color:lightgray;'>参数</div>");
//			div4.append("<div id='placeHolders' style='height:18px;width:40px;margin:3px;padding:3px;margin-left:25px;border:1px solid lightgray;display:none;'></div>");
//			div4.css({'min-height':'20px', 'min-width':'40px'}).find('#confHints').css('display',conf.lefts.length == 0?'block':'none');
			...
			table.find('img[paramItemName]').click(function(){
				aly.confField.initparammenu($(this).attr('paramItemName'));
			}).powerFloat({
				target : $("#confFieldFloat")
				,eventType:'click'
				,zIndex:50000
			...
			$('#allReportStyles').html('').append(styles);
			...
			if(desc == null) alert('luck of template rpx:'+conf.template);
			currDesc = desc.split(";");	
			...
			$('#modelReportFields').html('').append(table);
		...
		var changeFunc = function( event, ui ) {
			...
			//param布局位置错误
//			div4.find('#confHints').css("display",div4.find("div[iType=7]").length-div4.find('.ui-sortable-helper').length>0?"none":"block");
			...
			if (conf.type == 1) {
				...
				//param布局位置错误
//				div4.parent().attr('sel',0).css("background-color","");
				//console.log("----1");
		...
		if(!previewDbd){
			if (dataSet.type == 6) {
				...
				if(dataSet.fields == null) {
					console.log("跳过刷新，数据集发生变化，需要重新计算数据集字段");
				}else{
				...
				items.find('div[iType=1]').css({"cursor":"move"}).draggable({
					...
					,stop : function(event, ui) {
						...
						//param布局位置错误
	//					if (div4 != null) div4.attr("sel",0).parent().css("background-color","");
						for (var i=0; i<divs.length; i++) {
		...
		aly.cache.doms = {div1:div1,div2:div2,div3:div3,divs:divs};//div4:div4 param布局位置错误
...
//		}
		...
		,del : function() {//DBD
			...
			if(dataSet.type == 6 || dataSet.type == 7){
				...
				for(var i = 0; i < fields.length; i++){
					if(fields[i].name == aly.calcField.currItem){
						lmdTableObj.fields = fields.deleteIndex(i);
						aly.refresh();
						return;
					}
				}
	...
	,confField : {
		currField : null
		,initparammenu: function(f){
			aly.currParam = f;
			var conf = aly.getCurrRpx();
			var dataSet = aly.getDataSet(conf.dataSet);
			var field = aly.getRpxFieldByName(conf,f);
			var paramNames = aly.paramNames;
...
//			buts.find('[order="'+field.order+'"]').css('visibility','visible');
		...
		,init : function(f) {
			...
			buts.find('[seq=21]').css('display','none');
			buts.find('[seq=22]').css('display','none');
			buts.find('[aggr]').css('visibility','hidden');
			buts.find('[aggr="'+field.aggr+'"]').css('visibility','visible');
			buts.find('[order]').css('visibility','hidden');
			buts.find('[order="'+field.order+'"]').css('visibility','visible');
...
//						for (var i=0; i<fields.length; i++) fields[i] = transWhereInfo(fields[i],null,false,null,field.src == fields[i]? field.calc:null);
						for (var i=0; i<fields.length; i++){
							if(fields[i].split(";;;;").length != 2) {
		...
		,params_where : function(having){
			$.powerFloat.hide();
			//目前只有一个数据集
			var conf = aly.getCurrRpx();
			//alert(aly.confField.currField);
			var field = aly.getParamItem(aly.currParam);
			var dataSet = rqAnalyse.dataSets[0];
			if( dataSet.type < 6 ) var field_1_5 = aly.getDataSetField(dataSet,aly.currParam);
			if (dataSet.type < 6)
			{
				var dataScope = null;
				var func = function(data) {
				...
				aly.calcDfxData(field_1_5, conf, func);
				if (dataScope == null) return;
				//alert(dataScope+"111");
				var ed = defaultEdit.getEditStyle(field_1_5.srcEdit);
				if (ed != null) ed.dataScope = dataScope;
			}
			//alert(dataScope+"222");
			var filter1 = '';
			...
			if (dataSet.type == 6){
				...
				if (having == 2) {
					...
					} else if(field._fieldType == ''){
						fields = [transWhereInfo(field.src,null,false)];
					} else {
						fields = [transWhereInfo(field.src,field.name,false)];
					}
					
				}
			} else {
				fields[0] = {disp:field_1_5.name,dataType:field_1_5.dataType,edit:field_1_5.srcEdit,exp:field_1_5.src,valueType:1,values:""};
			}
			if (initField == null) initField = fields[0];
...
//		aly.refresh(true,true);
	...
	,refreshReport : function(confName, noCalc, noRefreshDialog, drillRpx, zongji) {
		...
		for (var i=0; i<rqAnalyse.rpxs.length; i++) {
			...
			if (dlg == null) {
				...
				if(!drillRpx)controlUtil.dashboardParse(currArea);
				initDialogOver = false;
				var newHeight = parseInt($(currArea).parent().css('height'))-20-divide;//减去area里bar和一点偏移
				dlg = art.dialog({
					id : dialogCount++,
				    ...
				    content: drillRpx == null ? '' : '<iframe name="'+confi.name+'" confName="'+confi.name+'" style="border:0;width:100%;height:'+newHeight+'"></iframe>'
				    ,ok : null
				 	,close: function () {
				 		if(drillRpx) {
				...
				reports[reports.length] = {area:currArea,dlg:dlg,name:confi.name};
				initDialogOver = true;
				//$('iframe[confName="'+confi.name+'"]').attr('src', contextPath + guideConf.guideDir+"jsp/empty.jsp?finalView="+finalView+"&guideDir="+guideConf.guideDir+"&confName="+encodeURIComponent(confi.name));
				if(!drillRpx)dlg.hide();
			} else {
				...
				if (guideConf.showHistoryRpx == 'yes' && confi._firstOpen) {
					...
					$.ajax({
						...
						success: function(data){
							...
							if (data == 1) {
								//var params;
								//for(var p = 0; p < aly.params.length; p++){
								//	if(p != 0) params += ';'
								//	var pj = aly.params[p];
								//	params += pj.name + '=' + pj.value;
								//}
								openHisory = true;
								console.log(guideConf.reportPage);
								$('iframe[confName="'+confi.name+'"]').attr('src', guideConf.reportPage+"?reportId="+confi.reportId+"&folded="+controlUtil.getAreaFolded(confi.name)+"&guideDir="+guideConf.guideDir+"&isOlap=no"+"&confName="+encodeURIComponent(confi.name)+"&finish=1&rpxFile="+encodeURIComponent(n));
								return;
							}
						}
					});
			...
			if ((!openHisory) && confi.autoCalc==1 && !noCalc) aly.queryData(confi,zongji);
	...
	,editCalcField : function(fieldName, callback) {
		...
		var dlg = art.dialog({
		    ...
		    content: '<div style="margin:10px;">'+resources.guide.js164+'：<input type="text" id="calcFieldName" value="'+(field==null?"":field.name)+'" style="width:120px;"></div>'
		    	...
		    	+'<div style="margin:10px;">'+resources.guide.js165+'：<select id="calcFieldType" style="width:120px;">'+raqDt.getDataTypeOptions(2)+'</select></div>'
	...
	,generateReport : function(conf, zongji) {
		...
		var success = function(data){
			if (data.indexOf('error:')==0) {
				...
				$('iframe[confName="'+conf.name+'"]').attr('src', "./empty.jsp?finalView="+finalView+"&?guideDir="+guideConf.guideDir+"&folded="+controlUtil.getAreaFolded(conf.name)+"&confName="+encodeURIComponent(conf.name));
				return;
			}
			$('iframe[confName="'+conf.name+'"]').attr('src', guideConf.reportPage+"?reportId="+conf.reportId+"&guideDir="+guideConf.guideDir+"&isOlap=no"+"&folded="+controlUtil.getAreaFolded(conf.name)+"&confName="+encodeURIComponent(conf.name)+"&finish="+finish);
		}
		var finish = conf._dataOver;

		var templateRpx = function() {
			if (conf._dataCount == 0) {
				$('iframe[confName="'+conf.name+'"]').attr('src', "./empty.jsp?finalView="+finalView+"&guideDir="+guideConf.guideDir+"&folded="+guideConf.guideDir+controlUtil.getAreaFolded(conf.name)+"&confName="+encodeURIComponent(conf.name));
				$('img[confNameLoading="'+conf.name+'"]').css('visibility','hidden');
				return;
			}
			...
			$.ajax({
				...
				async : true,
				url: contextPath + "/DLServletAjax?d="+new Date().getTime(),
				data: {action:2,pagerStyle:0,oper:'calcReport',reportId:conf.reportId,title:conf.name,reportType:conf.type,fields:fields,template:conf.template,resultRpxPrefixOnServer:guideConf.resultRpxPrefixOnServer},
				...
				success:function(data){
					...
					data = data.split(',');
					console.log(guideConf.reportPage);
					$('iframe[confName="'+data[0]+'"]').attr('src', guideConf.reportPage+"?reportId="+data[1]+"&guideDir="+guideConf.guideDir+"&isOlap=no"+"&folded="+controlUtil.getAreaFolded(data[0])+"&confName="+encodeURIComponent(data[0])+"&finish="+finish);
				}
			});
		}
		if (conf.type == 2) {
		...
		var olapRpx = function() {
			...
			$.ajax({
				...
				async : true,
				url: contextPath + "/DLServletAjax?d="+new Date().getTime(),
				data: {action:2,pagerStyle:0,zongji:zongji?true:false,oper:'genReport',reportId:conf.reportId,title:conf.name,rpt:JSON.stringify(rpt).replaceAll('"','<d_q>'),lefts:rpt.lefts,tops:rpt.tops,maxSize:guideConf.maxReportSize,resultRpxPrefixOnServer:guideConf.resultRpxPrefixOnServer,onlyRefreshDs:guideConf.onlyRefreshDs},
				//confs:[{type:1自定义分析报表/2模板报表,name:'报表名称',reportId:'',template:'',show:0/1,isRowData:1,lefts:[{name:'',item:itemId,use:1,order:0无序/1升序/2降序}],tops:[],fields:[],wheres:[{item:itemId}]}...] 调序、排序、改名				success: success
			});
		}
	...
	,queryData : function(conf,zongji) {
		...
		var dataSet = aly.getDataSet(conf.dataSet);
		if (dataSet.type == 6){
			var dql = rpxu.getDqlSegments(conf,aly.params,dataSet.params);
			if (dql == null) {
				return;
			}
			...
			$.ajax({
				...
				success: function(data){
					...
					if (data[0] == 0)
					{
						alert(conf.name+resources.guide.js170);
						return;
					}
					conf._dataOver = data[1];
					aly.generateReport(conf,zongji);
					//alert(data);
				}
			});
	...
	,calcDfxData : function(dataSetField, conf, callback) {
		var r = rpxu.getDfxExps(dataSetField, conf, aly.params);
		...
		function f() {
			$.ajax({
				...
				async : false,
				url: contextPath + "/DLServletAjax?d="+new Date().getTime(),
				data: {action:2,oper:'calc',reportId:reportId,dataId:dataId,cacheType:cacheType,calcs:calcs,filters:filters,fields:fields,resultExp:resultExp,types:r.types,srcTypes:r.srcTypes,dataFileType:guideConf.dataFileType,maxDataSize:guideConf.maxDataSize},
				success: function(data){
					if (data.indexOf('error:')==0) {
						alert(data.substring(6));
						return;
...
function reportCellDrill(reportConfName, topValues, leftValues, fieldValues, detail, zongji) {
	...
	for (var j=0; j<conf2.lefts.length ; j++ ) conf2.fields.push(conf2.lefts[j]);
	conf2.tops = [];
	conf2.lefts = [];
	var prex = conf2Name;
	if (prex != '') prex = conf2Name+"_";
...
//		var sArea = $(divCells[x]).find('.singleArea') 
//		if(sArea == null || sArea.length == 0){
//			areaId = addAreaIntoDivCell($(divCells[x]).attr('id'));
//			controlUtil.areas[controlUtil.areas.length-1].report = conf2.name;
//			break;
//		}
//	}
//	currArea = $("#"+areaId);
//	currArea.html("");
//	currArea.attr("confname",conf2.name);
//	currArea.attr("title",$(divCells[x]).attr("row")+"_"+$(divCells[x]).attr("col"));
	
	
	aly.refresh(null,null,conf2.name,null,null,zongji);
}
...
function saveFile(content,type,needDataPath,oldDataPath) {
	...
	var cb = function() {
		...
		if(type=='dbd') onServer = 'WEB-INF/files/dbd/';
	...
	var dlg = art.dialog({
	    ...
	    content: '<input id="saveGrpxName" type="text" value="'+guideConf.defaultSaveName+'" placeholder="'+resources.guide.js173+(type=='dbd'?'(.dbd)"':'(.olap)"')+' style="width:150px;margin:10px;">'
	    	...
	    	+ (previewDbd? '<script>$("#saveGrpxChk2").hide();</script>': "")
    	,button: [
	         {
	             ...
	             callback: function() {
					...
					if ($('#saveGrpxChk2')[0].checked) {
						...
						$.ajax({
							...
							async : false,
							url: contextPath + "/DLServletAjax?d="+new Date().getTime(),
							data: {action:2,oper:'fileExist',file:onServer, filetype:filetype},
							success: function(data){
								if (data.indexOf('error:')==0) {
									alert(data.substring(6));
									return;
...
function openFile(type,callback) {
	...
	var dlg = art.dialog({
		id : dialogCount++,
		title : resources.guide.js180+(type=='qyx'?resources.guide.js181:(type=='olap'?resources.guide.js182:(type=='fileData'?resources.guide.js183:''))),
	    content: ''
...
//			return;
		...
		if(f.indexOf('.ctx') != -1 || f.indexOf('.btx')){
...
function openDataFile(){
	openFile("fileData",function(content, file){
		...
		else if (file.toLowerCase().indexOf(".ctx")>=0) file = "return file(\""+file+"\").create().cursor()";
		else if (file.toLowerCase().indexOf(".btx")>=0) file = "return file(\""+file+"\").cursor@b()";
		else {
...
function saveOlap(){
	if(previewDbd)
		saveFile(aly.toString(null,$('#mframe')),"dbd",false,"");
	else
		saveFile(aly.toString(),"olap",false,"");
}

function saveOlapTemp(){
	var name = "temp"+new Date().getTime()+".olap";
	$('#downloadForm #path').val(guideConf.olapFolderOnServer+"/temp/"+name);
	$('#downloadForm #content').val(aly.toString());
	$('#downloadForm #mode').val("server");
	$('#downloadForm').submit();
	return name
}
...
function getLayoutRowAndCol(rname){
	var areas = controlUtil.areas;
	var area;
	var rc = null;
	for(var a = 0; a < areas.length; a++){
		area = areas[a];
		if(area.report == rname){
			rc = area.inDivCell;
			break;
		}
	}
	var rcs = rc.split("_");
	rcs = rcs.deleteIndex(0);
	return rcs;
}

function showAggrModels(){
	if($('#aggrModels').css('display') == "none") $('#aggrModels').show();
	else $('#aggrModels').hide();
}
